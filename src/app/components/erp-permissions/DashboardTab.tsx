...
const ROLES = [
  { id: 'owner', label: 'OWNER', level: 4, levelColor: 'text-[var(--erp-money-positive)]', icon: Crown, desc: 'Full company access, all branches, all modules, override everything' },
...
export function DashboardTab() {
  ...
  if (loading) return <div className="space-y-6"><h2 className="text-xl font-bold text-white">ERP Permission Architecture</h2><p className="text-muted-foreground text-sm">Loading dashboard…</p></div>;
  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-muted-foreground text-sm mt-1">Standard role-based permission system with branch-level access control</p>
      </div>
      ...
      <div className="grid grid-cols-4 gap-4">
        ...
        <div className="bg-muted/60 border border-border rounded-lg p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-2xl font-bold text-foreground">{branches}</p>
        ...
        <div className="bg-muted/60 border border-border rounded-lg p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-2xl font-bold text-foreground">{permissionsCount.allowed}/{permissionsCount.total || 58}</p>
      ...
      {/* Role Hierarchy */}
      <div>
        <h3 className="text-lg font-semibold text-foreground mb-4">Role Hierarchy</h3>
            ...
            >
              ...
              <div className="flex-1">
                <div className="flex items-center gap-2">
                  ...
                  <span className={cn('text-xs font-medium px-2 py-0.5 rounded', r.levelColor, 'bg-muted')}>
                    Level {r.level}
                  </span>
                </div>
                <p className="text-sm text-muted-foreground mt-0.5">{r.desc}</p>
              </div>
              <span className="text-sm text-muted-foreground">{userCountByRole[r.id] ?? 0} user(s)</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div>
        <h3 className="text-lg font-semibold text-foreground mb-4">4-Layer Permission Architecture</h3>
        <div className="grid grid-cols-4 gap-4">
          <div className="bg-muted/60 border border-border rounded-lg p-4">
            <p className="font-medium text-foreground mb-1">1. ROLE LAYER</p>
            <p className="text-sm text-muted-foreground">Owner, Admin, Manager, Salesman, User</p>
          </div>
          <div className="bg-muted/60 border border-border rounded-lg p-4">
            <p className="font-medium text-foreground mb-1">2. BRANCH ACCESS</p>
            <p className="text-sm text-muted-foreground">Multi-branch assignment control</p>
          </div>
          <div className="bg-muted/60 border border-border rounded-lg p-4">
            <p className="font-medium text-foreground mb-1">3. MODULE PERMISSION</p>
            <p className="text-sm text-muted-foreground">Sales, Payments, Ledger, Inventory</p>
          </div>
          <div className="bg-muted/60 border border-border rounded-lg p-4">
            <p className="font-medium text-foreground mb-1">4. VISIBILITY RULE</p>
            <p className="text-sm text-muted-foreground">Own vs Branch vs Company level</p>
          </div>
        </div>
      </div>
    </div>
