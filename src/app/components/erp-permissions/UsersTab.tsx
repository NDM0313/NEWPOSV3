...
const ROLE_META: Record<string, { label: string; icon: typeof Crown; color: string }> = {
  owner: { label: 'OWNER', icon: Crown, color: 'bg-green-500/20 text-[var(--erp-money-positive)] border-green-500/30' },
  ...
  user: { label: 'USER', icon: ShoppingCart, color: 'bg-gray-500/20 text-muted-foreground border-gray-500/30' },
};
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">Add, edit, and manage user roles and branch assignments</p>
        </div>
      ...
      <div className="flex gap-4">
        <div className="flex-1 relative">
          ...
          <input type="text" placeholder="Search by name or email..." value={search} onChange={(e) => setSearch(e.target.value)} className="w-full bg-muted border border-border rounded-lg pl-10 pr-4 py-2 text-white placeholder-gray-500" />
        </div>
        <select value={roleFilter} onChange={(e) => setRoleFilter(e.target.value)} className="bg-muted border border-border rounded-lg px-4 py-2 text-white">
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-border bg-muted/50">
              <th className="text-left text-muted-foreground font-medium p-4">User</th>
              <th className="text-left text-muted-foreground font-medium p-4">Role</th>
              <th className="text-left text-muted-foreground font-medium p-4">Assigned Branches</th>
              <th className="text-left text-muted-foreground font-medium p-4">Status</th>
              <th className="text-right text-muted-foreground font-medium p-4">Actions</th>
            </tr>
          </thead>
          <tbody>
            {filtered.map((u) => {
              ...
              return (
                <tr key={u.id} className="border-b border-border">
                  ...
                  <td className="p-4"><span className="text-muted-foreground text-xs">{userBranchNames[u.id] ?? '…'}</span></td>
                  <td className="p-4"><span className={u.is_active ? 'text-green-500 text-xs' : 'text-muted-foreground text-xs'}>{u.is_active ? 'Active' : 'Inactive'}</span></td>
                  <td className="p-4 text-right">
                    <button type="button" className="p-2 text-blue-400 hover:bg-muted rounded" onClick={() => { setEditingUser(u); setAddUserOpen(true); }} title="Edit"><Pencil size={16} /></button>
                    <button type="button" className="p-2 text-red-400 hover:bg-muted rounded" onClick={() => handleDelete(u)} disabled={deletingId === u.id} title="Deactivate"><Trash2 size={16} /></button>
                  </td>
                </tr>
              );
            })}
      ...
      <div className="grid grid-cols-4 gap-4">
        {Object.entries(ROLE_META).map(([key, meta]) => (
          <div key={key} className="bg-muted/60 border border-border rounded-lg p-4 text-center">
            <meta.icon className="mx-auto text-muted-foreground mb-2" size={24} />
            <p className="text-sm text-muted-foreground">Role: {meta.label}</p>
            <p className="text-lg font-bold text-foreground">{byRole[key] ?? 0} user(s)</p>
          </div>
        ))}
      </div>
