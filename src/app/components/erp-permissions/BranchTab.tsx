    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">Manage branches and user access across different locations</p>
        </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        {branches.map((b) => (
          <div key={b.id} className="bg-muted/60 border border-border rounded-lg overflow-hidden">
            <div className="bg-green-600/20 border-b border-green-500/30 p-4 flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <p className="font-semibold text-foreground">{b.name || 'Branch'}</p>
                  <p className="text-xs text-muted-foreground">{b.address || b.code || '-'}</p>
                </div>
              </div>
              <div className="flex items-center gap-2">
                <span className="text-xs text-muted-foreground">Code: {b.code || b.id.slice(0, 8)}</span>
                <button type="button" className="p-1.5 text-muted-foreground hover:bg-muted rounded" onClick={() => { setEditingBranch(b); setBranchModalOpen(true); }} title="Edit"><Pencil size={14} /></button>
                <button type="button" className="p-1.5 text-red-400 hover:bg-muted rounded" onClick={() => handleDelete(b)} disabled={deletingId === b.id} title="Delete"><Trash2 size={14} /></button>
              </div>
            </div>
            <div className="p-4">
              <p className="text-sm text-muted-foreground mb-2">Assigned Users ({(usersByBranch[b.id] || []).length})</p>
              <div className="space-y-2">
                {(usersByBranch[b.id] || []).map((u, i) => (
                  <div key={i} className="flex items-center gap-2 text-sm">
                    ...
                    <span className={cn('ml-auto px-2 py-0.5 rounded text-xs', u.role === 'admin' ? 'bg-blue-500/20 text-blue-400' : u.role === 'owner' ? 'bg-green-500/20 text-[var(--erp-money-positive)]' : 'bg-gray-600 text-muted-foreground')}>{u.role}</span>
                  </div>
                ))}
                {(usersByBranch[b.id] || []).length === 0 && <p className="text-muted-foreground text-sm">No users assigned</p>}
              </div>
            </div>
          </div>
        ))}
