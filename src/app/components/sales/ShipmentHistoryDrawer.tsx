...
export function ShipmentHistoryDrawer({
  ...
  return (
    <>
      ...
      <div
        className="fixed right-0 top-0 bottom-0 z-50 w-full max-w-md bg-muted/40 border-l border-border shadow-xl flex flex-col"
      ...
      >
        <div className="shrink-0 flex items-center justify-between px-4 py-3 border-b border-border">
          <h3 className="text-lg font-semibold text-foreground flex items-center gap-2">
            ...
            Shipment History
            {invoiceNo && (
              <span className="text-sm font-normal text-muted-foreground">— {invoiceNo}</span>
            )}
          </h3>
          <Button
            variant="ghost"
              ...
              {shipment && (
                <div className="space-y-3">
                  <div className="flex items-center gap-2 text-sm">
                    ...
                    <span className="text-foreground">{shipment.courier_name || '—'}</span>
                  </div>
                  <div className="flex items-center gap-2 text-sm">
                    <Hash size={16} className="text-muted-foreground" />
                    <span className="text-muted-foreground">Tracking:</span>
                    <span className="text-foreground font-mono">{shipment.tracking_id || '—'}</span>
                  </div>
                  <div className="grid grid-cols-1 gap-2 pt-2 border-t border-border">
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="text-[var(--erp-money-positive)]">{fmtCurrency(Number(shipment.charged_to_customer) || 0)}</span>
                    </div>
                    <div className="flex justify-between text-sm">
                    ...
                    {ledgerRow && (
                      <div className="flex justify-between text-sm">
                        <span className="text-muted-foreground">Courier Payable</span>
                    ...
                    {[...history].reverse().map((rec, i) => (
                      <div key={rec.id ?? i} className="flex gap-3">
                        <div className="flex flex-col items-center">
                          ...
                          {i < history.length - 1 && (
                            <div className="w-0.5 flex-1 min-h-[24px] bg-muted" />
                          )}
                        </div>
                        <div className="pb-4">
                          <p className="text-sm font-medium text-foreground">{rec.status}</p>
                          <p className="text-xs text-muted-foreground mt-0.5">{fmtDate(rec.created_at)}</p>
                        </div>
                      </div>
                    ))}
                  </div>
