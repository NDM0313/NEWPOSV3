      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-muted-foreground mt-1">{order.invoiceNumber} - {order.customerName}</p>
          </div>
          <Button
            variant="outline"
            onClick={onClose}
        ...
        <div className="bg-gradient-to-br from-purple-900/30 to-blue-900/30 border border-purple-500/30 rounded-xl p-6 mb-8">
          <div className="grid grid-cols-4 gap-6">
            <div>
              ...
              <div className="text-foreground font-semibold">{order.invoiceNumber}</div>
            </div>
            <div>
              <div className="text-xs text-muted-foreground mb-1">Customer</div>
              <div className="text-foreground font-semibold">{order.customerName}</div>
            </div>
            <div>
              <div className="text-xs text-muted-foreground mb-1">Fabric</div>
              <div className="text-foreground font-semibold">{order.fabric.type}</div>
            </div>
            <div>
              <div className="text-xs text-muted-foreground mb-1">Total Amount</div>
              <div className="text-foreground font-semibold">₹{order.totalAmount.toLocaleString()}</div>
            </div>
          </div>
        </div>
            ...
            {timeline.map((item, index) => {
              ...
              return (
                <div key={index} className="relative pl-20">
                  ...
                  {/* Content */}
                  <div className={`bg-card border-2 rounded-xl p-6 transition-all ${
                    isCompleted
                      ? 'border-green-500/30'
                      : isInProgress
                        ? 'border-blue-500/30'
                    ...
                    <div className="flex items-start justify-between mb-3">
                      <div>
                        <h3 className="text-lg font-bold text-foreground mb-1">{item.stage}</h3>
        ...
        <div className="mt-8 bg-card border border-border rounded-xl p-6">
          ...
          <div className="grid grid-cols-4 gap-4">
            <div className="text-center">
              <div className="text-2xl font-bold text-[var(--erp-money-positive)]">
                {timeline.filter(t => t.status === 'completed').length}
              </div>
              <div className="text-xs text-muted-foreground mt-1">Completed Stages</div>
            </div>
            <div className="text-center">
        ...
        {(order.customization.lace || order.customization.dyeing) && (
          <div className="mt-6 bg-card border border-border rounded-xl p-6">
            <h3 className="text-lg font-semibold text-foreground mb-4">Customization Details</h3>
            <div className="grid grid-cols-2 gap-6">
              {order.customization.lace && (
                <div>
                  <div className="text-sm text-muted-foreground mb-2">Lace</div>
                  <div className="text-foreground font-medium">{order.customization.lace.style}</div>
                  <div className="text-xs text-muted-foreground mt-1">Width: {order.customization.lace.width}</div>
                </div>
              )}
              {order.customization.dyeing && (
                <div>
                  <div className="text-sm text-muted-foreground mb-2">Dyeing</div>
                  <div className="text-foreground font-medium">{order.customization.dyeing.type}</div>
