      ...
      <div className="grid grid-cols-5 gap-4">
        <div className="bg-gradient-to-br from-purple-900/30 to-purple-900/10 border border-purple-900/50 rounded-xl p-4">
          <div className="flex items-center justify-between mb-2">
            <span className="text-xs text-muted-foreground uppercase font-medium">Dyeing (Dahair)</span>
            <div className="h-8 w-8 rounded-full bg-purple-500/20 flex items-center justify-center">
              <span className="text-purple-400 text-lg">🎨</span>
            </div>
          </div>
          <p className="text-3xl font-bold text-foreground">{stats.dyeingJobs}</p>
        ...
        <div className="bg-gradient-to-br from-pink-900/30 to-pink-900/10 border border-pink-900/50 rounded-xl p-4">
          <div className="flex items-center justify-between mb-2">
            <span className="text-xs text-muted-foreground uppercase font-medium">Handwork</span>
            <div className="h-8 w-8 rounded-full bg-pink-500/20 flex items-center justify-center">
              <span className="text-pink-400 text-lg">✋</span>
            </div>
          </div>
          <p className="text-3xl font-bold text-foreground">{stats.handworkJobs}</p>
        ...
        <div className="bg-gradient-to-br from-blue-900/30 to-blue-900/10 border border-blue-900/50 rounded-xl p-4">
          <div className="flex items-center justify-between mb-2">
            <span className="text-xs text-muted-foreground uppercase font-medium">Stitching (Tailor)</span>
            <div className="h-8 w-8 rounded-full bg-blue-500/20 flex items-center justify-center">
              <span className="text-blue-400 text-lg">✂️</span>
            </div>
          </div>
          <p className="text-3xl font-bold text-foreground">{stats.stitchingJobs}</p>
        ...
        <div className="bg-gradient-to-br from-red-900/30 to-red-900/10 border border-red-900/50 rounded-xl p-4">
          <div className="flex items-center justify-between mb-2">
            <span className="text-xs text-muted-foreground uppercase font-medium">Overdue</span>
            <AlertTriangle size={16} className="text-red-400" />
          </div>
          <p className="text-3xl font-bold text-foreground">{stats.overdueJobs}</p>
        ...
        <div className="bg-gradient-to-br from-green-900/30 to-green-900/10 border border-green-900/50 rounded-xl p-4">
          <div className="flex items-center justify-between mb-2">
            <span className="text-xs text-muted-foreground uppercase font-medium">Completed</span>
            <CheckCircle2 size={16} className="text-[var(--erp-money-positive)]" />
          </div>
          <p className="text-3xl font-bold text-foreground">{stats.completedJobs}</p>
          <p className="text-xs text-[var(--erp-money-positive)] mt-1">Total Done</p>
        </div>
      </div>
      ...
      <div className="bg-card border border-border rounded-xl p-4">
        <div className="flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button
              ...
              className={cn(
                "border-border text-muted-foreground h-10",
                hasActiveFilters && "border-pink-500 text-pink-400 bg-pink-900/10"
              )}
            >
              <Filter size={16} className="mr-2" />
              Filter
              {hasActiveFilters && <Badge className="ml-2 bg-pink-500 text-foreground text-[10px] px-1.5 py-0">ON</Badge>}
            </Button>

            {/* Export Button */}
            <Button variant="outline" size="sm" className="border-border text-muted-foreground h-10">
        ...
        {filterOpen && (
          <div className="mt-4 pt-4 border-t border-border grid grid-cols-3 gap-4">
            ...
            {/* Priority Filter */}
            <div>
              <label className="text-xs text-muted-foreground uppercase font-medium mb-2 block">Priority</label>
              <select
      ...
      <div className="border border-border rounded-xl overflow-hidden bg-muted/40">
        ...
        <div className="bg-card/70 px-4 py-2 border-b border-border flex items-center justify-between">
          <p className="text-xs text-muted-foreground">
            Showing <span className="text-foreground font-medium">{displayedJobs.length}</span> of <span className="text-foreground font-medium">{filteredJobs.length}</span> jobs
          </p>
          {hasActiveFilters && (
                ...
                return (
                  ...
                  >
                    ...
                    {columnVisibility.jobId && (
                      <td className="p-4">
                        <div>
                          <p className="font-mono font-bold text-foreground">{job.jobId}</p>
                          <p className="text-xs text-muted-foreground font-mono">{job.linkedInvoice}</p>
                          {isUrgent && (
                      ...
                      <td className="p-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-muted-foreground font-mono">{job.customerMobile}</p>
                          </div>
                        </div>
                      </td>
                    )}
                    ...
                    {columnVisibility.product && (
                      <td className="p-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="font-medium text-foreground">{job.productName}</p>
                            <p className="text-xs text-muted-foreground">{job.productCode}</p>
                          </div>
                        </div>
                      </td>
                    )}
                    ...
                    {columnVisibility.currentStep && (
                      <td className="p-4">
                        <div>
                          <p className="text-foreground font-medium">{job.currentStep}</p>
                          {job.subStatus && (
                    ...
                    {columnVisibility.assignedWorker && (
                      <td className="p-4">
                        <div className="flex items-center gap-2">
                          {job.assignedWorker !== '-' ? (
                            <>
                              <div className="h-8 w-8 rounded-full bg-gradient-to-tr from-orange-500 to-pink-500 flex items-center justify-center text-xs font-bold text-foreground">
                                {job.assignedWorker.charAt(0)}
                              </div>
                              <span className="text-muted-foreground text-sm">{job.assignedWorker}</span>
                            </>
                          ) : (
                    ...
                    {columnVisibility.deliveryDate && (
                      <td className="p-4">
                        <div className={cn(
                          ...
                          !isDelayed && !isNearDue && "text-muted-foreground"
                        )}>
                    ...
                    {columnVisibility.customerBilling && (
                      <td className="p-4 text-right">
                        <div className="text-[var(--erp-money-positive)] font-medium">₹{job.customerBilling.toLocaleString()}</div>
                        {columnVisibility.internalCost && (
                      ...
                      <td className="p-4">
                        <Badge 
                          ...
                          className={cn(
                            ...
                            job.status === 'Draft' && "bg-card/20 text-muted-foreground border-gray-900/50",
                            ...
                            job.status === 'Ready' && "bg-green-900/20 text-[var(--erp-money-positive)] border-green-900/50",
                            job.status === 'Delivered' && "bg-purple-900/20 text-purple-400 border-purple-900/50",
                            job.status === 'Closed' && "bg-card/20 text-muted-foreground border-gray-900/50"
                          )}
                        >
                          {job.status}
                        </Badge>
                        ...
                        <div className="flex items-center justify-end gap-2">
                          ...
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end" className="bg-card border-border text-foreground w-56">
                              <DropdownMenuItem onClick={() => handleAction(job, 'view')} className="hover:bg-muted cursor-pointer">
                                <Eye size={14} className="mr-2" />
                                View Job Card
                              </DropdownMenuItem>
                              <DropdownMenuItem onClick={() => handleAction(job, 'workflow')} className="hover:bg-muted cursor-pointer">
                                <Zap size={14} className="mr-2" />
                                Edit Workflow
                              </DropdownMenuItem>
                              <DropdownMenuSeparator className="bg-muted" />
                              <DropdownMenuItem onClick={() => handleAction(job, 'assign')} className="hover:bg-muted cursor-pointer">
                                <UserPlus size={14} className="mr-2" />
                                Assign Worker
                              </DropdownMenuItem>
                              <DropdownMenuItem onClick={() => handleAction(job, 'cost')} className="hover:bg-muted cursor-pointer">
                                <DollarSign size={14} className="mr-2" />
                                Add Cost
                              </DropdownMenuItem>
                              <DropdownMenuItem onClick={() => handleAction(job, 'complete')} className="hover:bg-muted cursor-pointer">
                                <CheckCircle2 size={14} className="mr-2" />
                                Mark Step Complete
                              </DropdownMenuItem>
                              <DropdownMenuSeparator className="bg-muted" />
                              <DropdownMenuItem onClick={() => handleAction(job, 'invoice')} className="hover:bg-muted cursor-pointer text-[var(--erp-money-positive)]">
                                <FileText size={14} className="mr-2" />
                                Generate Invoice
                              </DropdownMenuItem>
                              <DropdownMenuItem onClick={() => handleAction(job, 'close')} className="hover:bg-muted cursor-pointer text-red-400">
