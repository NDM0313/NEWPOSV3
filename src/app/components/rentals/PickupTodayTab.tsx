...
export const PickupTodayTab = ({ onProcessPickup, refreshKey = 0, activeCount = 0, onViewAllRentals }: PickupTodayTabProps) => {
  ...
  const renderCard = (r: RentalUI, isMissed: boolean) => (
    <div
      key={r.id}
      className="bg-card border border-border rounded-xl p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4 hover:border-border transition-colors"
    >
      <div className="flex items-start gap-4 min-w-0 flex-1">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="text-foreground font-medium mt-1">{r.customerName}</p>
          <div className="mt-2 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2 text-sm text-muted-foreground">
            {r.customerContact && (
      ...
      <Button
        size="lg"
        className="bg-green-600 hover:bg-green-500 text-foreground font-semibold shadow-lg shadow-green-600/20 shrink-0"
      ...
      <div className="shrink-0 mb-4">
        ...
        <div className="mt-4 flex flex-wrap gap-3">
          <div className="rounded-lg border border-border bg-muted/50 px-4 py-2">
            ...
            <p className="text-xl font-bold text-foreground">{kpis.dueToday}</p>
          </div>
          <div className="rounded-lg border border-border bg-muted/50 px-4 py-2">
            <span className="text-xs text-muted-foreground uppercase">Missed</span>
          ...
          <div className="rounded-lg border border-border bg-muted/50 px-4 py-2 min-w-0">
            <span className="text-xs text-muted-foreground uppercase">Balance Due</span>
      ...
      {loading ? (
        ...
        <div className="flex-1 overflow-y-auto space-y-6">
          ...
          {dueToday.length > 0 && (
            <section>
              {missed.length > 0 && (
                <h4 className="text-sm font-semibold text-foreground mb-3">Due Today ({dueToday.length})</h4>
              )}
