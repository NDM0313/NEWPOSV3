...
export function VariationAttributesMaster() {
  ...
  if (!companyId) {
    return <p className="text-sm text-muted-foreground">Select a company to manage variation attributes.</p>;
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap gap-2 items-end bg-input-background/80 border border-border rounded-lg p-4">
        <div className="flex-1 min-w-[200px]">
          <Label className="text-muted-foreground">New attribute name</Label>
          <Input
            value={newAttr}
        ...
        <Button type="button" variant="secondary" onClick={addAttribute} className="bg-muted text-foreground">
      ...
      <div className="space-y-4">
        {attrNames.length === 0 && (
          <p className="text-sm text-muted-foreground border border-dashed border-border rounded-lg p-6 text-center">
            No attributes yet. Add names like <span className="text-muted-foreground">Color</span> or{' '}
            <span className="text-muted-foreground">Size</span>, then add values for each.
          </p>
        )}
        {attrNames.map((attr) => (
          <div key={attr} className="border border-border rounded-xl p-4 bg-muted/40">
            ...
            <div className="flex flex-wrap gap-2 mb-3">
              {(data[attr] || []).map((v) => (
                <span
                  key={v}
                  className="inline-flex items-center gap-1 px-2 py-1 rounded-md bg-muted text-sm text-gray-200 border border-border"
                >
                  {v}
                  <button type="button" className="text-muted-foreground hover:text-red-400" onClick={() => removeValue(attr, v)} aria-label={`Remove ${v}`}>
                    ×
                  </button>
                </span>
              ))}
            ...
            <div className="flex gap-2 flex-wrap">
              ...
              />
              <Button type="button" size="sm" variant="secondary" className="bg-muted text-foreground" onClick={() => addValue(attr)}>
                <Plus className="h-4 w-4 mr-1" />
                Add value
              </Button>
            </div>
