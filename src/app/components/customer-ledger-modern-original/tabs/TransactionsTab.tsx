              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-gradient-to-r from-slate-100 to-slate-50 border-b-2 border-slate-300">
                    {visibleColumns.date && (
                      <th 
                        className="px-5 py-4 text-left text-xs font-bold text-slate-700 cursor-pointer hover:bg-slate-200 transition-colors uppercase tracking-wider"
                    ...
                    {visibleColumns.debit && (
                      <th 
                        className="px-5 py-4 text-right text-xs font-bold text-slate-700 cursor-pointer hover:bg-slate-200 transition-colors uppercase tracking-wider"
                ...
                <tbody>
                  {filteredTransactions.map((transaction, index) => (
                    <tr
                      ...
                      className={`border-b border-slate-100 hover:bg-gradient-to-r hover:from-blue-50 hover:to-transparent transition-all cursor-pointer group ${
                        selectedTransaction?.id === transaction.id ? 'bg-blue-50 ring-2 ring-blue-300 ring-inset' : index % 2 === 0 ? 'bg-white' : 'bg-slate-50/40'
                      }`}
                    >
                      {visibleColumns.date && (
                        ...
                        <td className="px-5 py-4">
                          <div className="flex items-center gap-2">
                            ...
                            {transaction.linkedInvoices && transaction.linkedInvoices.length > 0 && (
                              <div className="px-1.5 py-0.5 bg-blue-100 rounded text-xs text-blue-700 flex items-center gap-1" title="Linked invoices">
                      ...
                      {visibleColumns.paymentAccount && (
                        <td className="px-5 py-4">
                          <div className="flex items-center gap-2 text-sm text-slate-700">
                            ...
                            <span className="font-medium">{transaction.paymentAccount}</span>
                          </div>
                        </td>
                      )}
                      {visibleColumns.debit && (
