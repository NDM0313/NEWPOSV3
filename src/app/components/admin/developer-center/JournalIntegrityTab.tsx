    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-end gap-2">
        <div className="flex-1 min-w-[180px]">
          <label className="text-[10px] uppercase tracking-wider text-gray-500">Filter (q)</label>
          <Input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="JE-0012 or payment" className="mt-1 bg-gray-950 border-gray-800" />
        </div>
        <div>
        ...
        <Card className="border-gray-800 bg-gray-900/40">
          ...
          <CardContent className="overflow-x-auto">
            {snapshot.rows.length === 0 ? (
              <p className="text-sm text-gray-500 py-4 text-center">No journals match filters.</p>
            ) : (
              <table className="w-full text-xs">
                ...
                <tbody>
                  {snapshot.rows.map((row) => (
                    <tr key={row.je.id} className="border-b border-gray-800/60 align-top">
                      <td className="py-2 pr-2 text-gray-200">{row.je.entry_no || row.je.id.slice(0, 8)}</td>
                      <td className="py-2 pr-2 text-gray-400">{row.je.entry_date?.slice(0, 10)}</td>
                      <td className="py-2 pr-2 text-gray-500">{row.je.reference_type}</td>
                      <td className="py-2 pr-2 text-gray-300">{row.uiRef?.displayRef || '—'}</td>
                      <td className="py-2 pr-2">{severityBadge(row.severity)}</td>
                      <td className="py-2 text-gray-400 max-w-md">{row.summary}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
