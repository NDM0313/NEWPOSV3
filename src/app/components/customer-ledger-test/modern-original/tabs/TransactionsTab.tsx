...
export function TransactionsTab({ transactions, saleItemsMap = new Map(), studioDetailsMap = new Map(), onTransactionClick, openingBalance: openingBalanceProp = 0, accountName = 'Account', dateRange: dateRangeProp }: TransactionsTabProps) {
  ...
  const [sortOrder, setSortOrder] = useState<SortOrder>('asc'); // Standard ledger: oldest (start) at top, newest (end) at bottom
  ...
  // Period debit/credit exclude the synthetic Opening Balance row so totals match getLedgerSummary / ModernSummaryCards (opening is its own card).
