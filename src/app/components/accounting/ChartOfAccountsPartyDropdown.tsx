...
export function ChartOfAccountsPartyDropdown({
  ...
  return (
    <div
      className={cn(
        'border-t border-border bg-input-background/90',
    ...
    >
      <div className="flex items-start justify-between gap-3 mb-2">
        <div className="flex items-center gap-2 min-w-0">
          <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-muted/90 border border-border/80">
          ...
          <div className="min-w-0">
            ...
            {!isLinkedOnly && scopeLabel ? (
              <p className="text-[10px] text-muted-foreground mt-0.5 leading-snug">
                Journal lines on <strong className="text-muted-foreground">one GL account id</strong> per control code (RPC), not
                the whole subtree. Contacts primary balances ={' '}
                <code className="text-muted-foreground">get_contact_balances_summary</code>.
              </p>
            ) : null}
            {isLinkedOnly ? (
              <p className="text-sm text-foreground font-medium truncate mt-0.5">{linkedContactName}</p>
            ) : null}
        ...
        <Button
          ...
          variant="ghost"
          size="sm"
          className="h-8 shrink-0 text-muted-foreground hover:text-foreground hover:bg-muted gap-1.5 px-2"
          onClick={onCollapse}
        >
      ...
      {!isLinkedOnly && !loading && !error && partyRows.length === 0 ? (
        <p className="text-sm text-muted-foreground py-2">No non-zero party balances on this control for the current scope.</p>
      ) : null}

      {!isLinkedOnly && !loading && partyRows.length > 0 ? (
        <ul className="rounded-lg border border-border/90 bg-muted/40 divide-y divide-border/80 max-h-[min(320px,50vh)] overflow-y-auto">
          {partyRows.map((r) => {
      ...
      {!isLinkedOnly && glParity && glParity.controlTrialBalance != null ? (
        <div className="mt-3 text-[10px] text-muted-foreground space-y-1 border-t border-border pt-2.5">
          <p className="font-semibold text-muted-foreground uppercase tracking-wide">Balance trace (GL)</p>
          <p>
            COA row balance (may include sub-account roll-up):{' '}
            <span className="tabular-nums text-muted-foreground">{formatCurrency(glParity.coaRowDisplayBalance)}</span>
          </p>
          <p>
            Trial balance — this control account id (Dr − Cr):{' '}
            <span className="tabular-nums text-muted-foreground">{formatCurrency(glParity.controlTrialBalance)}</span>
          </p>
          ...
          <p>
            {['2010', '1180'].includes(String(glParity.controlCodeLabel || '').trim()) ? (
              <>
                ...
                <span className="tabular-nums text-muted-foreground">{formatCurrency(glParity.partyAttributedSumFull)}</span>
              </>
            ) : (
              <>
          ...
          {glParity.unmappedTop && glParity.unmappedTop.length > 0 ? (
            <div className="pt-1">
              <p className="text-[9px] font-semibold text-muted-foreground uppercase tracking-wide">Top unmapped (by |amount|)</p>
              <ul className="mt-0.5 space-y-0.5">
                {glParity.unmappedTop.map((u) => (
                  <li key={u.referenceType} className="flex justify-between gap-2 tabular-nums text-muted-foreground">
