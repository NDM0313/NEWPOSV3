...
export function classifyOrphanArReversalDefect(
  ...
  return {
    ...
    recommendedAction: 'Run GL correction dry-run, review Dr/Cr preview, then apply only after migration approval',
    primaryButton: 'create_gl_correction_draft',
    canApply: false,
    riskLevel: 'high',
    blockReason: 'GL correction apply requires migration RPC — dry-run available now',
    queueItem: {
