...
/** Format send note line for DB storage (RPC appends to notes). */
export function formatSendNotes(text: string): string {
  return text.trim();
}

/** Format receive note line for DB storage (RPC appends to notes). */
...
/** Notes without workflow/task prefix lines (assignment / misc). */
