...
export type BarcodeLabelPreset = {
  ...
  /** When set, selecting this preset enables fixed sticker size (mm). */
  labelWidthMm?: number;
  labelHeightMm?: number;
  useFixedLabelSize?: boolean;
};

/** A4 printable area defaults for fit helper. */
...
export function fitLabelsOnA4Page(opts: {
  ...
  // UI column buttons are 2–4; allow 1 only if physically required, else clamp for UI.
  const a4Columns = Math.max(2, Math.min(4, cols === 1 ? 2 : cols));
  // Recalc rows if we forced columns up (rare for tiny stickers).
...
export function stickerSizeSummary(widthMm: number, heightMm: number, columns: number, maxPerSheet: number): string {
