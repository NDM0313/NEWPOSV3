/**
 * Build label print rows for Products catalog and Purchase PO lines (web).
 */
...
function mergePacking(existing: string | undefined, incoming: string | undefined): string | undefined {
  ...
  return lines.map((line) => {
    ...
    if (line.variationId) {
      ...
      if (v) {
        ...
        if (vCode) next = { ...next, barcode: vCode, sku: v.sku || next.sku || vCode };
        if (v.name) next = { ...next, variationName: v.name };
        if (v.retail_price != null) next = { ...next, price: Number(v.retail_price) };
      }
    }
    return next;
  });
...
/** Map purchase_items row from getPurchase() to label lines input. */
export function purchaseItemsToLabelInput(items: any[]): PurchaseItemForLabels[] {
  return (items || []).map((i: any) => {
    const rawPd = i.packing_details;
    const pd: PackingDetailsForLabels | undefined = rawPd
      ? {
          total_boxes: rawPd.total_boxes ?? rawPd.total_boxes,
          total_pieces: rawPd.total_pieces,
          total_meters: rawPd.total_meters,
          packs: rawPd.packs ?? rawPd.total_boxes,
          units_per_pack: rawPd.units_per_pack,
        }
      : undefined;
    return {
      id: String(i.id),
      productId: i.product_id || i.product?.id,
      variationId: i.variation_id ?? null,
