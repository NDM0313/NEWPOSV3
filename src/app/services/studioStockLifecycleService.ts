...
/**
 * Ensures a single PRODUCTION_IN movement exists for the studio-generated product linked to this sale.
 * Uses production stage costs (same basis as invoice sync). Safe to call repeatedly.
 */
export async function ensureStudioProductionInForSale(saleId: string): Promise<{ inserted: boolean; skippedReason?: string }> {
  ...
  const productions = await studioProductionService.getProductionsBySaleId(saleId);
  if (!productions?.length) return { inserted: false, skippedReason: 'no_production' };

  const production = productions[0] as {
    id: string;
  ...
  const { data: items } = await supabase
    ...
    .eq('sale_id', saleId);
  const studioItem = (items || []).find((i: { is_studio_product?: boolean | null }) => i.is_studio_product === true);
  ...
  let productionCost = sumProductionCostFromStages(stages || []);
  ...
  if (insErr) {
    console.warn('[studioStockLifecycleService] PRODUCTION_IN insert failed:', insErr.message);
