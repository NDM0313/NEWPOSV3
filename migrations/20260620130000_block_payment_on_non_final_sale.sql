-- Block payment GL posting on non-final sales (prevents order-stage AR credit without invoice JE).
...
BEGIN
  ...
  IF p_reference_type = 'sale' THEN
    ...
    IF v_sale_record.id IS NOT NULL THEN
      IF lower(trim(v_sale_record.status::text)) <> 'final' THEN
        RETURN json_build_object(
          'success', false,
          'error', 'Cannot post payment GL for non-final sale. Finalize the sale first.'
        );
      END IF;
...
COMMENT ON FUNCTION public.record_payment_with_accounting(UUID, UUID, payment_type, VARCHAR, UUID, DECIMAL, payment_method_enum, DATE, UUID, VARCHAR, TEXT, UUID) IS
  'Payment posting: sale/purchase/rental/expense/on_account. Sale payments require final status.';
