-- Supplier AP statement: same party attribution as get_contact_party_gl_balances (2000 control only).
-- Replaces client-side supplierApJournalLineMatchesSupplier which missed payment-linked JEs when
-- payments.contact_id was null but purchase graph still resolves supplier (G-PAR-02b).
...
AS $BODY$
  ...
  for_supplier AS (
    SELECT * FROM je_lines WHERE party_id = p_supplier_id
  ),
  opening_calc AS (
...
COMMENT ON FUNCTION public.get_supplier_ap_gl_ledger_for_contact(UUID, UUID, UUID, DATE, DATE) IS
  'AP (code 2000) lines for one supplier using _gl_resolve_party_id_for_journal_entry — parity with get_contact_party_gl_balances. Returns JSON: period_opening_balance + rows.';
