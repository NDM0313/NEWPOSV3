-- Fix: expenses.status is expense_status enum; JSONB -> text COALESCE was not assignable.
-- Replaces create_expense_document only; same logic, explicit enum cast on status.
