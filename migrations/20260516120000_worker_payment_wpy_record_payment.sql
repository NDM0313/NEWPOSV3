-- ============================================================================
-- Worker payments: WPY-* refs, padded JE strings clamped, record_payment_with_accounting(worker_payment)
-- Extends 20260513120001 generate_document_number + 20260512100000 record_payment_with_accounting.
-- ============================================================================

ALTER TABLE public.journal_entries
  ADD COLUMN IF NOT EXISTS document_no VARCHAR(100) NULL;

-- Cap insane padding values left by legacy bugs (prevents JE-00000000001-style strings).
UPDATE public.erp_document_sequences
SET padding = LEAST(GREATEST(COALESCE(padding, 4), 4), 6),
    updated_at = now()
WHERE padding IS NOT NULL AND padding > 6;
...
COMMENT ON FUNCTION public.erp_document_default_prefix(TEXT) IS
  'Default prefix per document type. WORKER_PAYMENT → WPY (studio/worker pay vouchers).';

CREATE OR REPLACE FUNCTION public._sync_worker_payment_sequence_after_duplicate(p_company_id UUID, p_year INTEGER)
RETURNS VOID
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  VALUES (
    ...
    v_sentinel,
    'WORKER_PAYMENT',
    public.erp_document_default_prefix('worker_payment'),
    p_year,
    0,
    4,
    now()
  ...
  WHERE e.company_id = p_company_id
    ...
    AND e.document_type = 'WORKER_PAYMENT'
    AND e.year = p_year;
END;
$syncw$;

COMMENT ON FUNCTION public._sync_worker_payment_sequence_after_duplicate(UUID, INTEGER) IS
  'Align WORKER_PAYMENT sentinel last_number with max WPY-* numeric suffix after duplicate key.';
...
COMMENT ON FUNCTION public.generate_document_number(UUID, UUID, TEXT, BOOLEAN) IS
  'ERP numbering: WORKER_PAYMENT/WPY; padding clamped 4–6; STUDIO self-heal; JV/FT.';
  ...
  LOOP
    ...
    EXCEPTION
      WHEN unique_violation THEN
        IF lower(trim(COALESCE(p_reference_type, ''))) = 'worker_payment' THEN
          PERFORM public._sync_worker_payment_sequence_after_duplicate(p_company_id, v_year);
          UPDATE public.erp_document_sequences
...
COMMENT ON FUNCTION public.record_payment_with_accounting(UUID, UUID, payment_type, VARCHAR, UUID, DECIMAL, payment_method_enum, DATE, UUID, VARCHAR, TEXT, UUID, UUID) IS
  'Payment posting: sale/purchase/rental/expense/on_account/worker_payment; WPY refs; JE mirrors voucher no.';
