-- Runs after get_contact_balances_summary_rpc.sql (alphabetically). Idempotent CREATE OR REPLACE.
-- Worker payables: unpaid worker_ledger_entries + workers.current_balance fallback (not contacts.current_balance only).
...
COMMENT ON FUNCTION public.get_contact_balances_summary(UUID, UUID) IS
  'Per-contact receivables/payables. Workers: unpaid worker_ledger sum if ledger rows exist; else workers.current_balance; else contact fields.';
