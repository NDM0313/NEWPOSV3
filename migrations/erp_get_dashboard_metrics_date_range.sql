-- Issue 05: Dashboard cards synchronized with selected date range and single source.
-- get_dashboard_metrics: when p_start_date/p_end_date provided, override period metrics
-- (monthly_revenue, monthly_expenses, monthly_profit) and trends for that range.
-- Safe: read-only, same SECURITY DEFINER pattern.
...
BEGIN
  ...
  -- 1) Base financial metrics (today/month from existing RPC)
  BEGIN
  ...
  -- Detect date columns (same as get_financial_dashboard_metrics)
  ...
  -- 2) Period metrics for selected date range (canonical: sales, purchases, expenses tables)
  EXECUTE format(
  ...
  -- Receivables/Payables: canonical from sales/purchases (outstanding due)
  SELECT COALESCE(SUM(due_amount), 0) INTO v_receivables
  FROM sales WHERE company_id = p_company_id AND status = 'final' AND COALESCE(due_amount, 0) > 0;
  SELECT COALESCE(SUM(due_amount), 0) INTO v_payables
  FROM purchases WHERE company_id = p_company_id AND status IN ('final', 'received') AND COALESCE(due_amount, 0) > 0;

  -- Override metrics with period values so dashboard cards match selected range
  ...
  -- Period "today" display: use period totals when range is selected
  ...
  -- 3) Trend arrays for each day in [v_start, v_end]
  ...
  -- 4) Sales by category (same as before)
...
COMMENT ON FUNCTION get_dashboard_metrics(UUID, UUID, DATE, DATE) IS
  'Issue 05: Dashboard metrics for selected date range; single source for cards (sales, purchases, expenses).';
