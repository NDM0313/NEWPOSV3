-- Stock movements from update_sale_with_items only when sale is final (matches web posting gates).
-- Prevents studio orders / draft / quotation from getting sale OUT rows on mobile edit.
...
BEGIN
    ...
    IF v_sale.status = 'final'::public.sale_status THEN
