-- Bespoke WO complete: CUSTOM parent stock posts as +IN (purchase Received style).
-- Fabric stays -OUT. One-shot repair flips legacy negative parent rows.

SET search_path = public;

-- Repair existing parent OUT rows before RPC replace (idempotent)
UPDATE public.stock_movements
SET quantity = ABS(quantity),
    total_cost = ABS(total_cost),
    notes = REPLACE(notes, 'Bespoke custom order OUT', 'Bespoke custom order IN')
WHERE reference_type = 'bespoke_work_order'
  AND notes LIKE 'Bespoke custom order OUT%'
  AND quantity < 0;
...
BEGIN
  ...
  -- Parent custom-order line IN (+order qty on CUSTOM/generic SKU — ledger visibility)
  SELECT
    si.product_id,
    si.variation_id,
    si.quantity,
...
COMMENT ON FUNCTION public.complete_bespoke_work_order(uuid, uuid) IS
  'Complete bespoke WO: Dr 5000 / Cr tailor AP + idempotent fabric OUT + parent custom-order stock IN.';
