-- Tailor/dyer on sale extras = expense_categories sub-row (not supplier contact).
-- Extends 4120 clearing RPCs; keeps tailor_contact_id for legacy + bespoke.
...
COMMENT ON COLUMN public.sale_charges.expense_category_id IS
  'Tailor/dyer sub-category under Stitching/Dying (expense_categories); preferred over tailor_contact_id for sale extras.';

-- ----------------------------------------------------------------------------
-- extra_service_clearing_lines — category id + name
-- ----------------------------------------------------------------------------
...
BEGIN
  ...
  SELECT
    ...
    sc.expense_category_id,
    COALESCE(ec.name, c.name),
