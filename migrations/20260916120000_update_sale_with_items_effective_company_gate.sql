-- Fix update_sale_with_items company gate for platform company session.
-- Home users.company_id stays on the operator's home tenant; sale edits must
-- authorize against get_user_company_id() (session override) like RLS.
-- Body otherwise matches 20260602130000 (bespoke parent / relink).
...
BEGIN
    ...
    -- Effective company (platform session override) must match the sale tenant.
    IF public.get_user_company_id() IS DISTINCT FROM v_sale.company_id THEN
...
COMMENT ON FUNCTION public.update_sale_with_items(
    UUID, JSONB, UUID, NUMERIC, NUMERIC, NUMERIC, NUMERIC, TEXT, TEXT, TEXT, TEXT, DATE, DATE, UUID
) IS
  'Edit sale lines + header. Company gate uses get_user_company_id() (platform session aware).';
