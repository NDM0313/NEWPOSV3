-- ============================================================================
-- STUDIO → SALES COST: ONLY COMPLETED STAGES
-- ============================================================================
-- Studio cost on sales = SUM(cost) from studio_production_stages
-- WHERE production linked to sale AND status = 'completed'.
-- Ensures Sales tab shows studio cost only after stages are completed.
-- ============================================================================

-- 1. get_sale_studio_charges: only sum completed stages (used by sync trigger)
...
COMMENT ON FUNCTION get_sale_studio_charges(UUID) IS 'Sum of completed stage costs for productions linked to sale. Used by sync trigger.';

-- 2. get_sale_studio_charges_batch: only sum completed stages (for Sales list)
...
-- 3. Re-sync all sales that have studio productions (so studio_charges reflects completed-only)
DO $$
DECLARE
  r RECORD;
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.routines WHERE routine_schema = 'public' AND routine_name = 'sync_sale_studio_charges_for_sale') THEN
    FOR r IN SELECT DISTINCT p.sale_id FROM studio_productions p WHERE p.sale_id IS NOT NULL
    LOOP
