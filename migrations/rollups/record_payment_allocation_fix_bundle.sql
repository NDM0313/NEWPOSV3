-- ============================================================================
-- BUNDLE: record_payment_with_accounting fixes (paste entire file in SQL Editor)
-- ============================================================================
-- Order: savepoint split -> company-scoped PAY -> sentinel repair -> duplicate-proof -> on_account branches
-- Files: 20260509180000, 20260509190000, 20260510100000, 20260511120000, 20260512100000
-- After run: migrations/diagnostics/payment_reference_allocation_check.sql
-- ============================================================================

-- ============================================================================
-- record_payment_with_accounting: persist ERP sequence bumps across INSERT retries
-- ============================================================================
-- Problem: When generate_document_number() and INSERT lived in the same inner
-- BEGIN...EXCEPTION block, a unique_violation on INSERT rolled back the whole
-- block â€” including erp_document_sequences updates â€” so every retry produced the
-- same PAY-xxxx and callers fell through to timestamp-hash â€œfallbackâ€ refs.
--
-- Fix: Allocate reference_number in its own block *before* the INSERT block so
-- sequence advances survive failed inserts; retries get the next PAY-nnnn.
...
BEGIN
  ...
  -- Allocate ref outside INSERT exception block so erp_document_sequences bumps persist on retry.
  LOOP
    v_try := v_try + 1;
...
'Atomic payment posting: PAY-nnnn via generate_document_number; ref allocation outside INSERT savepoint so sequence advances persist on unique_violation retries.';
...
-- ============================================================================
-- 20260512100000 on_account (full replace of record_payment_with_accounting)
-- ============================================================================
-- See migrations/20260512100000_record_payment_with_accounting_on_account.sql
