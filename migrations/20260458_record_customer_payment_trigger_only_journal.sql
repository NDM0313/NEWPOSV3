-- ============================================================================
-- record_customer_payment: single GL path via AFTER INSERT ON payments trigger
-- ============================================================================
-- Web inserts payments only; create_payment_journal_entry (trigger) posts Dr/Cr.
-- Mobile RPC must NOT insert journal_entries (that duplicated trigger postings).
--
-- After insert: locate JE by payment_id, patch Cr AR line from parent 1100 (same
-- resolution as create_payment_journal_entry) to _ensure_ar_subaccount_for_contact,
-- mirroring web patchPaymentJeToSubLedger.
--
-- Sale totals: removed redundant UPDATE sales — recalc_sale_payment_totals (AFTER
-- INSERT on payments) already sets paid_amount/due_amount/payment_status.
--
-- Requires: AFTER INSERT trigger on payments → create_payment_journal_entry,
-- _ensure_ar_subaccount_for_contact, generate_document_number (optional).
-- ============================================================================
      ...
      IF v_try <= 8 THEN
        ...
        EXCEPTION
          WHEN undefined_function THEN
            v_ref_no := public.get_next_document_number(p_company_id, v_branch_id, 'payment');
          WHEN OTHERS THEN
            v_ref_no := public.get_next_document_number(p_company_id, v_branch_id, 'payment');
        END;
      ELSE
...
COMMENT ON FUNCTION public.record_customer_payment(UUID, UUID, UUID, NUMERIC, UUID, TEXT, DATE, TEXT, UUID) IS
  'Customer receipt: INSERT payments only; trigger posts JE; AR sub-ledger patch matches web.';
