-- Fix ensure_sale_stock_movements: legacy sale_items used column "price" which no longer
-- exists (schema is unit_price). Also stop falling through to sale_items when sales_items
-- exists but all lines were intentionally filtered (track_stock=false / CUSTOM / bespoke).
-- Same one-line rename on handle_sale_final_stock_movement legacy branch for consistency.
...
BEGIN
  ...
  IF EXISTS (
    ...
    -- sales_items is canonical; do not fall through to legacy sale_items when rows
    -- exist but were filtered (non-tracked / CUSTOM / bespoke defer).
...
COMMENT ON FUNCTION public.ensure_sale_stock_movements(uuid) IS
  'Idempotent sale OUT from sales_items; skips bespoke deferred lines; legacy sale_items uses unit_price.';

-- Legacy sale_items branch: price → unit_price (dead when sales_items exists, still rotten).
...
COMMENT ON FUNCTION public.handle_sale_final_stock_movement() IS
  'Stock OUT on sale final. Skips bespoke deferred lines; legacy sale_items uses unit_price.';
