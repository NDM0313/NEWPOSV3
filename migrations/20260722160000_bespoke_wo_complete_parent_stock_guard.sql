...
BEGIN
  ...
  -- Parent custom-order line IN — only CUSTOM-* or fabric-linked parents (not plain retail SKUs)
  SELECT
    si.product_id,
    si.variation_id,
    si.quantity,
  ...
  THEN
    ...
    IF NOT EXISTS (
      ...
      INSERT INTO public.stock_movements (
        ...
        v_qty, v_unit_cost, v_qty * v_unit_cost,
        'sale', 'bespoke_work_order', p_work_order_id,
        'Bespoke custom order IN — ' || COALESCE(v_wo.work_order_no, p_work_order_id::text) || ' — ' || v_name,
        p_user_id, now()
      );
      v_stock_posted := v_stock_posted + 1;
    END IF;
