-- ============================================================================
-- record_customer_payment: harden reference_number allocation against collisions
-- ============================================================================
-- Why:
-- - Under retries/concurrency, reference_number can still collide with
--   payments_reference_number_unique in some deployments.
-- - Keep numbering aligned with ERP engine by preferring
--   generate_document_number(..., 'payment', false).
--
-- What:
-- - Replace record_customer_payment with unique-violation retry loop
--   around payment insert.
-- - Try ERP numbering engine first; fallback to get_next_document_number.
-- - Keep existing accounting and AR-subaccount behavior unchanged.
-- ============================================================================
...
COMMENT ON FUNCTION public.record_customer_payment(UUID, UUID, UUID, NUMERIC, UUID, TEXT, DATE, TEXT, UUID) IS
  'Atomic customer payment with ERP numbering-first reference allocation and unique-violation retry guard.';
