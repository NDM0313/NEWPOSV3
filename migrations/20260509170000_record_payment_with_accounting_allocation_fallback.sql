-- ============================================================================
-- record_payment_with_accounting: collision-proof reference after first failure
-- ============================================================================
-- If generate_document_number keeps returning an existing payments.reference_number,
-- repeated unique_violations exhaust retries. After the first failed INSERT, use a
-- timestamp + hash suffix reference so the next attempt succeeds without masking
-- upstream numbering drift (still fix erp_document_sequences separately).
...
BEGIN
  ...
  -- INSERT payments: ERP number first; after any unique_violation use collision-proof ref (retry budget raised).
  LOOP
    v_try := v_try + 1;
    BEGIN
      ...
      ELSIF v_try >= 2 THEN
        v_ref_no := 'PAY-' || to_char(clock_timestamp(), 'YYYYMMDDHH24MISSMS') || '-' ||
...
'Atomic payment posting: ERP payment ref + unique-violation retries; after first collision uses PAY-timestamp-hash fallback (v15 retries).';
