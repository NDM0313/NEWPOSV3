-- Roznamcha / unified cash-bank: do not treat AP/liability names containing "cash"
-- (e.g. "Payable — CASH PURCHASE") as liquidity. Name heuristics only for
-- cash/bank/wallet/asset (or standard 10xx codes).
...
DECLARE
  ...
  v_name_heuristic_ok boolean := FALSE;
BEGIN
  ...
  IF v_code IN ('1000', '1010', '1020') THEN
    v_is_liq := TRUE;
  ELSIF length(v_digits) >= 4 AND v_digits LIKE '100%' THEN
    v_is_liq := TRUE;
  ELSIF length(v_digits) >= 3 AND v_digits LIKE '101%' THEN
    v_is_liq := TRUE;
  ELSIF length(v_digits) >= 3 AND v_digits LIKE '102%' THEN
    v_is_liq := TRUE;
  ELSIF v_type IN ('cash', 'bank', 'mobile_wallet', 'wallet', 'card', 'pos') THEN
    v_is_liq := TRUE;
  ELSIF v_name ~* 'cash|bank|mobile wallet|wallet|jazz|easypaisa|ndm|easy\s*paisa|mobicash|finja|upaisa|sadapay|nayapay' THEN
    -- Name match must not promote AP/expense/inventory (e.g. Payable — CASH PURCHASE)
    IF v_type IN (
      'liability', 'equity', 'revenue', 'income', 'expense', 'cogs',
      'cost_of_goods_sold', 'inventory', 'receivable', 'payable'
    ) OR v_type LIKE '%liability%' OR v_type LIKE '%payable%' THEN
      v_is_liq := FALSE;
    ELSE
      v_name_heuristic_ok := (
        v_type = ''
        OR v_type IN ('asset', 'other_asset', 'current_asset', 'other_current_asset',
                      'cash', 'bank', 'mobile_wallet', 'wallet', 'card', 'pos')
      );
      v_is_liq := v_name_heuristic_ok;
    END IF;
  END IF;
...
COMMENT ON FUNCTION public._unified_ledger_is_liquidity_account(text, text, text, text) IS
  'Liquidity filter for unified ledger. Excludes party T/T, TT agent RMB wallets, and non-liquidity types whose names contain cash/bank (e.g. Payable — CASH PURCHASE).';
