-- Forward migration: prevent duplicate sales_items_pkey during restore_company_backup_rpc.
-- Deletes backup PK ids + all lines for tenant sales immediately before sales_items insert (orphan/stale cleanup).
-- Full CREATE OR REPLACE keeps deployed DBs aligned without editing prior migration history.
