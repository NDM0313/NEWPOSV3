-- Invoice document JSON: include bespoke_parent_item_id on line items when column exists.
...
DECLARE
  ...
  v_has_custom_col BOOLEAN := FALSE;
  v_has_parent_col BOOLEAN := FALSE;
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'sales_items') THEN
    IF v_has_custom_col AND v_has_parent_col THEN
      SELECT jsonb_agg(
        jsonb_build_object(
          'id', i.id,
...
COMMENT ON FUNCTION generate_invoice_document(UUID) IS 'Invoice document JSON; items include customization_details and bespoke_parent_item_id when columns exist.';
