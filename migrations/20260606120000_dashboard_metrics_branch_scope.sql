-- Dashboard period metrics: respect p_branch_id on sales, purchases, expenses, trends, category, low stock.
-- AR/AP unchanged (already branch-scoped via get_contact_balances_summary).
-- Cash/bank remain company-wide (GL as-of today).
...
BEGIN
  ...
  v_metrics := jsonb_set(v_metrics, '{period_scope}', to_jsonb((CASE WHEN p_branch_id IS NULL THEN 'all_branches' ELSE 'branch' END)::text));
  v_metrics := jsonb_set(v_metrics, '{cash_bank_scope}', to_jsonb('company'::text));

  FOR d IN SELECT generate_series(v_start, v_end, '1 day'::interval)::date
  LOOP
    EXECUTE format(
      'SELECT COALESCE(SUM(total), 0) FROM sales WHERE company_id = $1
         AND LOWER(TRIM(COALESCE(status::text, %L))) = %L AND (%I)::date = $2
         AND ($3::uuid IS NULL OR branch_id = $3)',
      '', 'final', sales_col
    ) INTO day_sales USING p_company_id, d, p_branch_id;
    EXECUTE format(
      'SELECT COALESCE(SUM(total), 0) FROM purchases WHERE company_id = $1
         AND LOWER(TRIM(COALESCE(status::text, %L))) IN (%L, %L) AND (%I)::date = $2
         AND ($3::uuid IS NULL OR branch_id = $3)',
      '', 'final', 'received', purch_date_col
    ) INTO day_pur USING p_company_id, d, p_branch_id;
    EXECUTE format(
      'SELECT COALESCE(SUM(amount), 0) FROM expenses WHERE company_id = $1
         AND LOWER(TRIM(COALESCE(status::text, %L))) = %L AND (%I)::date = $2
         AND ($3::uuid IS NULL OR branch_id = $3)',
      '', 'paid', exp_date_col
    ) INTO day_exp USING p_company_id, d, p_branch_id;
...
COMMENT ON FUNCTION public.get_dashboard_metrics(UUID, UUID, DATE, DATE) IS
  'Dashboard pack: period sales/purchases/expenses/trends/category/low-stock respect p_branch_id; AR/AP branch-scoped; cash/bank company-wide.';
