...
BEGIN
  ...
  -- Control account 2030 must exist
  SELECT id INTO v_control_id
  FROM accounts
  WHERE company_id = p_company_id AND code = '2030' AND is_active = true
  LIMIT 1;
  ...
  -- If we have a contact, look for existing courier payable account for this contact
  IF p_contact_id IS NOT NULL THEN
    SELECT id INTO v_account_id
    FROM accounts
    WHERE company_id = p_company_id AND contact_id = p_contact_id AND is_active = true
    LIMIT 1;
  ...
  -- Next code: 2031, 2032, 2033 ... (max of existing 203x codes + 1)
  SELECT COALESCE(MAX(
    CASE
      WHEN code ~ '^203[0-9]+$' AND LENGTH(code) <= 4 THEN (SUBSTRING(code FROM 4))::INT
      WHEN code ~ '^203[0-9]+$' THEN (SUBSTRING(code FROM 4))::INT
      ELSE 0
    END
  ), 0) + 1 INTO v_max_suffix
  FROM accounts
  WHERE company_id = p_company_id AND code ~ '^203[0-9]+$';
  ...
  VALUES (
    p_company_id,
    v_next_code,
    COALESCE(TRIM(p_contact_name), 'Courier') || ' Payable',
    'Liability',
    0,
    true,
    v_control_id,
...
EXCEPTION
  WHEN unique_violation THEN
    -- Concurrent insert; fetch by contact_id again
    SELECT id INTO v_account_id FROM accounts
    WHERE company_id = p_company_id AND contact_id = p_contact_id AND is_active = true LIMIT 1;
    RETURN v_account_id;
END;
$$;

COMMENT ON FUNCTION get_or_create_courier_payable_account IS 'Returns account id for courier payable sub-ledger (2031, 2032...). Creates one per contact if missing.';
...
CREATE INDEX IF NOT EXISTS idx_sale_shipments_courier_id
  ON sale_shipments (courier_id) WHERE courier_id IS NOT NULL;
