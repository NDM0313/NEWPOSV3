-- Fail-closed AR/AP control posting for named parties (mobile rental fix).
-- Replaces silent 1100/2000 fallback when contact_id is known on received/paid payments.
-- Adds BEFORE INSERT/UPDATE trigger on journal_entry_lines for operational documents.

CREATE OR REPLACE FUNCTION public._is_account_control_code(p_account_id UUID, p_control_code TEXT)
RETURNS BOOLEAN
LANGUAGE sql
STABLE
...
AS $$
  SELECT EXISTS (
    ...
    WHERE a.id = p_account_id
      AND trim(COALESCE(a.code, '')) = trim(COALESCE(p_control_code, ''))
  );
$$;

COMMENT ON FUNCTION public._is_account_control_code(UUID, TEXT) IS
  'True when account_id is the AR (1100) or AP (2000) control row by code.';

-- Resolve party contact from parent journal entry (operational docs only).
CREATE OR REPLACE FUNCTION public._resolve_je_party_contact_id(p_je_id UUID)
RETURNS UUID
LANGUAGE plpgsql
STABLE
SET search_path = public
...
DECLARE
  ...
  v_contact_id UUID;
BEGIN
  SELECT reference_type, reference_id, payment_id
    INTO v_je
  ...
  IF lower(trim(COALESCE(v_je.reference_type, ''))) IN (
    'gl_correction', 'manual', 'adjustment', 'sale_adjustment', 'purchase_adjustment'
  ) OR COALESCE(v_je.reference_type, '') LIKE 'opening_balance%' THEN
    RETURN NULL;
  END IF;

  v_contact_id := NULL;

  IF v_je.reference_type = 'rental' AND v_je.reference_id IS NOT NULL THEN
    SELECT r.customer_id INTO v_contact_id
    FROM public.rentals r
    WHERE r.id = v_je.reference_id;
  ELSIF v_je.reference_type = 'sale' AND v_je.reference_id IS NOT NULL THEN
    SELECT s.customer_id INTO v_contact_id
    FROM public.sales s
    WHERE s.id = v_je.reference_id;
  ELSIF v_je.reference_type = 'purchase' AND v_je.reference_id IS NOT NULL THEN
    SELECT p.supplier_id INTO v_contact_id
    FROM public.purchases p
    WHERE p.id = v_je.reference_id;
  ELSIF lower(trim(COALESCE(v_je.reference_type, ''))) = 'payment' AND v_je.payment_id IS NOT NULL THEN
    SELECT pay.contact_id INTO v_contact_id
    FROM public.payments pay
    WHERE pay.id = v_je.payment_id;
  END IF;
...
CREATE OR REPLACE FUNCTION public.trg_reject_control_party_jel()
RETURNS TRIGGER
LANGUAGE plpgsql
SET search_path = public
AS $$
...
BEGIN
  ...
  IF v_code NOT IN ('1100', '2000') THEN
    RETURN NEW;
  END IF;

  v_contact_id := public._resolve_je_party_contact_id(NEW.journal_entry_id);
  IF v_contact_id IS NULL THEN
    RETURN NEW;
  END IF;

  RAISE EXCEPTION
    'Operational posting for named party (contact %) must use party sub-ledger, not control account %',
    v_contact_id, v_code;
END;
$$;

DROP TRIGGER IF EXISTS trg_reject_control_party_jel ON public.journal_entry_lines;
CREATE TRIGGER trg_reject_control_party_jel
BEFORE INSERT OR UPDATE OF account_id ON public.journal_entry_lines
FOR EACH ROW
EXECUTE FUNCTION public.trg_reject_control_party_jel();

-- ─── record_payment_with_accounting: fail closed for named party on control AR/AP ───
...
COMMENT ON FUNCTION public.record_payment_with_accounting IS
  'Payment posting; named party must use AR-CUS*/AP-SUP* sub-ledgers (fail-closed on control 1100/2000).';
