-- Rental wear/devaluation: Dr Rental Income (4200), Cr expense (5300 / fallback 6100).
-- Credits revenue accounts incorrectly increased income when 4200 was the credit line; debit reduces net rental income.
-- Does NOT touch cash/asset accounts or party AR.
-- Idempotent via caller-supplied action_fingerprint (rental_party_devaluation:*).
...
CREATE OR REPLACE FUNCTION public.record_rental_expense_devaluation_journal(
  ...
  p_debit_account_code    TEXT DEFAULT '4200',
  p_credit_account_code   TEXT DEFAULT '5300'
)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  v_primary_debit := NULLIF(trim(COALESCE(p_debit_account_code, '4200')), '');
  IF v_primary_debit IS NULL THEN
    v_primary_debit := '4200';
  END IF;

  v_primary_credit := NULLIF(trim(COALESCE(p_credit_account_code, '5300')), '');
  IF v_primary_credit IS NULL THEN
    v_primary_credit := '5300';
  END IF;

  -- Debit leg: Rental Income (4200) — reduces net revenue when debited.
  ...
  IF v_debit_id IS NULL THEN
    RETURN jsonb_build_object(
      ...
      'skipped', false,
      'error',
      'debit account not found for code ' || v_primary_debit || ' (expected Rental Income 4200)'
    );
  END IF;

  -- Credit leg: expense (5300), fallback 6100.
  ...
  IF v_credit_id IS NULL AND v_primary_credit = '5300' THEN
    SELECT a.id, a.name INTO v_credit_id, v_credit_name
    FROM public.accounts a
    WHERE a.company_id = v_company_id
  ...
  IF v_credit_id IS NULL THEN
    RETURN jsonb_build_object(
      ...
      'credit account not found for codes ' || v_primary_credit ||
        CASE WHEN v_primary_credit = '5300' THEN ' / 6100' ELSE '' END
    );
  END IF;
...
COMMENT ON FUNCTION public.record_rental_expense_devaluation_journal(
  ...
  'Rental wear: Dr Rental Income (4200), Cr expense (5300↔6100). Idempotent via p_action_fingerprint.';
