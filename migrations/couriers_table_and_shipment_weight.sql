...
-- ----------------------------------------------------------------------------
-- 1. Create couriers table (PART 3)
-- ----------------------------------------------------------------------------
...
CREATE POLICY couriers_select ON couriers FOR SELECT
  USING (company_id = get_user_company_id());

DROP POLICY IF EXISTS couriers_insert ON couriers;
CREATE POLICY couriers_insert ON couriers FOR INSERT
  WITH CHECK (company_id = get_user_company_id());

DROP POLICY IF EXISTS couriers_update ON couriers;
CREATE POLICY couriers_update ON couriers FOR UPDATE
  USING (company_id = get_user_company_id());

DROP POLICY IF EXISTS couriers_delete ON couriers;
CREATE POLICY couriers_delete ON couriers FOR DELETE
  USING (company_id = get_user_company_id());

-- ----------------------------------------------------------------------------
-- 2. sale_shipments: weight (PART 1) + courier_master_id (PART 2/9)
-- ----------------------------------------------------------------------------
ALTER TABLE sale_shipments
  ADD COLUMN IF NOT EXISTS weight NUMERIC(10,2);

ALTER TABLE sale_shipments
  ADD COLUMN IF NOT EXISTS courier_master_id UUID REFERENCES couriers(id) ON DELETE SET NULL;

CREATE INDEX IF NOT EXISTS idx_sale_shipments_courier_master ON sale_shipments(courier_master_id)
  WHERE courier_master_id IS NOT NULL;
...
-- ----------------------------------------------------------------------------
-- 3. couriers.account_id – link to payable account (PART 1: auto-created on courier create)
-- ----------------------------------------------------------------------------
