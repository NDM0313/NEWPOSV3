-- Customers & Suppliers report: Due column = party GL clamped at p_end_date (same as Contacts UI).
-- Activity columns remain period-scoped.
...
COMMENT ON FUNCTION public.get_customers_suppliers_report(UUID, DATE, DATE, UUID, TEXT, TEXT) IS
  'Customers & Suppliers report: period activity columns; Due = party GL follow-up (MAX(0, signed)) as at p_end_date — same basis as Contacts.';
