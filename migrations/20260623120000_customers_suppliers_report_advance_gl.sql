-- Customers & Suppliers report: add Advance (GL) = MAX(0, -signed party GL) at p_end_date.
-- Due (GL) remains follow-up only (MAX(0, signed)); activity columns unchanged.

DROP FUNCTION IF EXISTS public.get_customers_suppliers_report(UUID, DATE, DATE, UUID, TEXT, TEXT);
...
BEGIN
  ...
  party_gl AS (
    SELECT
      pg.contact_id AS cid,
      GREATEST(0, COALESCE(pg.gl_ar_receivable, 0))::numeric AS ar_clamped,
      GREATEST(0, COALESCE(pg.gl_ap_payable, 0) + COALESCE(pg.gl_worker_payable, 0))::numeric AS ap_clamped,
      GREATEST(0, -COALESCE(pg.gl_ar_receivable, 0))::numeric AS ar_advance,
      GREATEST(0, -(COALESCE(pg.gl_ap_payable, 0) + COALESCE(pg.gl_worker_payable, 0)))::numeric AS ap_advance
    FROM public.get_contact_party_gl_balances(p_company_id, p_branch_id, p_end_date) pg
  ),
  base AS (
    SELECT
      c.id AS contact_id,
      ...
      (
        CASE
          WHEN c.type = 'customer' THEN COALESCE(pgl.ar_clamped, 0)
          WHEN c.type IN ('supplier', 'worker') THEN COALESCE(pgl.ap_clamped, 0)
          WHEN c.type = 'both' THEN COALESCE(pgl.ar_clamped, 0) + COALESCE(pgl.ap_clamped, 0)
          ELSE 0::numeric
        END
      )::numeric AS due,
      (
        CASE
          WHEN c.type = 'customer' THEN COALESCE(pgl.ar_advance, 0)
          WHEN c.type IN ('supplier', 'worker') THEN COALESCE(pgl.ap_advance, 0)
          WHEN c.type = 'both' THEN COALESCE(pgl.ar_advance, 0) + COALESCE(pgl.ap_advance, 0)
          ELSE 0::numeric
        END
      )::numeric AS advance_gl
    FROM contacts c
    ...
    LEFT JOIN party_gl pgl ON pgl.cid = c.id
  ...
  WHERE
    ...
    AND (
      ...
      OR (v_balance_status = 'has_due' AND (ABS(b.due) > 0.009 OR b.advance_gl > 0.009))
      OR (v_balance_status = 'cleared' AND ABS(b.due) <= 0.009 AND b.advance_gl <= 0.009)
    )
...
COMMENT ON FUNCTION public.get_customers_suppliers_report(UUID, DATE, DATE, UUID, TEXT, TEXT) IS
  'Customers & Suppliers report: period activity columns; Due = party GL follow-up MAX(0, signed); Advance = credit/advance MAX(0, -signed) as at p_end_date.';
