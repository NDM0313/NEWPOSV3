-- Customer operational ledger: include sales that only have created_at (NULL invoice_date),
-- match final status case-insensitively, return shipment + discount/tax/expenses for UI detail.
-- SECURITY DEFINER bypasses branch RLS so the ledger matches GL-visible activity for the party.
...
COMMENT ON FUNCTION get_customer_ledger_sales(UUID, UUID, DATE, DATE) IS
  'Customer ledger sales: final only; effective date = invoice_date or UTC created_at date; includes shipment/discount/tax/expenses.';
