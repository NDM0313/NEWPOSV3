-- Wire worker_payment debit to role-model WA/WP leaves via _resolve_worker_payment_debit_account.
-- Additive: replaces worker debit resolution only inside record_payment_with_accounting (... p_worker_stage_id).
-- Requires: migrations/20260922120000_worker_courier_role_model_account_domain.sql
-- Does NOT rewrite historical journals. Staging/production apply is separate owner decision.
...
COMMENT ON FUNCTION public.record_payment_with_accounting(UUID, UUID, payment_type, VARCHAR, UUID, DECIMAL, payment_method_enum, DATE, UUID, VARCHAR, TEXT, UUID, UUID) IS
  'Payment posting; worker_payment debits WA-*/WP-* via _resolve_worker_payment_debit_account (no bare 1180 fallback).';
