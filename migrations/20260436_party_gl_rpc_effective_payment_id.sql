-- PF-14 payment_adjustment JEs store the payments.id in journal_entries.reference_id, not journal_entries.payment_id.
-- Party GL RPCs only exposed payment_id → UI could not resolve payments.payment_account_id (Cash G140) for JE-0039-style rows.
-- Fix: expose COALESCE(payment_id, reference_id::uuid when payment_adjustment) as payment_id in JSON rows.
...
COMMENT ON FUNCTION public.get_customer_ar_gl_ledger_for_contact(UUID, UUID, UUID, DATE, DATE) IS
  'AR lines for one customer: 1100 subtree. payment_id in JSON includes PF-14 payment_adjustment via reference_id when journal.payment_id is null.';
...
COMMENT ON FUNCTION public.get_supplier_ap_gl_ledger_for_contact(UUID, UUID, UUID, DATE, DATE) IS
  'AP lines for one supplier: 2000 subtree. payment_id in JSON includes PF-14 payment_adjustment via reference_id when journal.payment_id is null.';
