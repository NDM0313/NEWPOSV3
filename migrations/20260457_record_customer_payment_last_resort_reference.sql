-- ============================================================================
-- record_customer_payment: last-resort reference when ERP numbering exhausts retries
-- ============================================================================
-- Depends on:
--   - 20260450_record_customer_payment_reference_retry.sql (retry loop + RPC shape)
--   - 20260455_self_healing_numbering_and_payment_retry.sql (self-healing
--     generate_document_number — apply before this if collisions persist)
--
-- Why:
--   Some deployments still hit unique_violation on payments.reference_number
--   for all 8 ERP-issued attempts (sequence drift, concurrency, or legacy data).
--
-- What:
--   Attempts 1–8: unchanged — generate_document_number / get_next_document_number.
--   Attempts 9–12: use collision-resistant PAY-FB-* reference (timestamp + uuid),
--   then INSERT. Only after 12 failed inserts do we return the user-facing error.
-- ============================================================================
...
COMMENT ON FUNCTION public.record_customer_payment(UUID, UUID, UUID, NUMERIC, UUID, TEXT, DATE, TEXT, UUID) IS
  'Atomic customer payment: ERP payment refs (8 tries) then PAY-FB-* fallback refs (4 tries) before failure.';
