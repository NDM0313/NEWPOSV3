-- Extend create_business_transaction: branch city/state + fiscal dates on insert; seed accounting_settings fiscalYearEnd from param
...
EXCEPTION WHEN OTHERS THEN
  RAISE NOTICE 'branch_location: skip old signature drop: %', SQLERRM;
END $$;
...
BEGIN
  ...
  v_fiscal_end := COALESCE(
    p_fiscal_year_end::DATE,
    (v_fiscal_start + INTERVAL '1 year' - INTERVAL '1 day')::DATE
  );
  ...
  INSERT INTO branches (
    ...
    address, phone, country, city, state, fiscal_year_start, fiscal_year_end
  )
  VALUES (
    gen_random_uuid(),
    v_company_id,
    ...
    NULLIF(TRIM(COALESCE(p_branch_state, '')), ''),
    v_fiscal_start,
    v_fiscal_end
  )
  ...
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'settings') THEN
    ...
    (
      ...
      jsonb_build_object(
        ...
        'fiscalYearEnd', to_char(v_fiscal_end, 'YYYY-MM-DD'),
...
DO $$
BEGIN
  GRANT EXECUTE ON FUNCTION public.create_business_transaction(
    TEXT, TEXT, TEXT, TEXT, UUID, TEXT, DATE, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, JSONB,
    TEXT, TEXT, NUMERIC, TEXT, BOOLEAN, TEXT, JSONB, TEXT, TEXT, DATE
  ) TO service_role;
  GRANT EXECUTE ON FUNCTION public.create_business_transaction(
    TEXT, TEXT, TEXT, TEXT, UUID, TEXT, DATE, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, JSONB,
    TEXT, TEXT, NUMERIC, TEXT, BOOLEAN, TEXT, JSONB, TEXT, TEXT, DATE
...
EXCEPTION WHEN OTHERS THEN
  RAISE NOTICE 'branch_location: grants skipped: %', SQLERRM;
END $$;
