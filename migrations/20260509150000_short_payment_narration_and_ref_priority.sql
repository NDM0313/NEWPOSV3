-- Future-only short narration formatter for payment/receipt postings.
-- Does not backfill historical rows.

CREATE OR REPLACE FUNCTION public.extract_bank_trace_from_notes(p_notes TEXT)
RETURNS TEXT
LANGUAGE plpgsql
STABLE
AS $$
...
BEGIN
  ...
  SELECT (regexp_match(p_notes, '(?i)bank\s*trace\s*id\s*:\s*([^|]+)'))[1]
    INTO v_match;

  RETURN NULLIF(btrim(COALESCE(v_match, '')), '');
END;
$$;
...
BEGIN
  ...
  IF v_clean_note IS NOT NULL THEN
    -- If note is already in short format, keep only the user note tail to avoid duplication.
    IF v_clean_note ~* '^(Receipt|Payment)\s+.+\([^)]+\)\s*-\s*' THEN
      v_clean_note := btrim(regexp_replace(v_clean_note, '^(Receipt|Payment)\s+.+\([^)]+\)\s*-\s*', '', 'i'));
    END IF;
    -- Remove legacy long auto prefixes if caller still sends old narration.
...
BEGIN
  IF lower(COALESCE(NEW.reference_type, '')) <> 'payment' THEN
    RETURN NEW;
  END IF;
...
DROP TRIGGER IF EXISTS trg_sync_payment_short_narration_to_journal_entry ON public.journal_entries;
CREATE TRIGGER trg_sync_payment_short_narration_to_journal_entry
BEFORE INSERT ON public.journal_entries
FOR EACH ROW
EXECUTE FUNCTION public.sync_payment_short_narration_to_journal_entry();
