...
-- ============================================================================
-- Call from the app (Studio Costs tab) so accounts are created in authenticated
-- context. Safe to call multiple times (ON CONFLICT DO NOTHING).
-- Uses ensure_erp_accounts if it exists; otherwise inserts directly (without balance if column missing).
-- ============================================================================
...
COMMENT ON FUNCTION ensure_erp_accounts_for_current_company() IS
  'Ensures accounts 2000, 4000, 5000, 2010 for current user company. Call from Studio Costs tab.';

-- ============================================================================
-- RPC: Ensure ERP accounts for ALL companies (run by script or on demand)
-- ============================================================================
...
COMMENT ON FUNCTION ensure_erp_accounts_all_companies() IS
  'Ensures accounts 2000, 4000, 5000, 2010 for every company. Idempotent. Call from script or admin.';
