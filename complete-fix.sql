-- Complete fix - Drop all triggers, add columns, recreate triggers

BEGIN;

-- Step 1: Ensure default company
...
-- Step 2: Drop ALL triggers that might conflict
...
DROP TRIGGER IF EXISTS trigger_update_contact_balance_on_sale ON sales;
...
DROP TRIGGER IF EXISTS trigger_update_contact_balance_on_purchase ON purchases;
...
BEGIN
    ...
    -- Sales (now safe - triggers dropped)
    ...
    -- Purchases (now safe - triggers dropped)
...
-- Step 4: Recreate triggers (from functions.sql - they already exist, just recreate)
