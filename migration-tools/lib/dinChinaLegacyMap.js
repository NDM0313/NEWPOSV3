...
export async function findExistingLegacySale(supabase, companyId, legacyTransactionId, cache = null) {
  ...
  const { data: bySource } = await supabaseRead(`sales_dup_notes_${legacyTransactionId}`, () =>
    supabase
      .from('sales')
...
export async function findExistingLegacyPurchase(supabase, companyId, legacyTransactionId, cache = null) {
  ...
  const { data: byNotes } = await supabaseRead(`purchases_dup_notes_${legacyTransactionId}`, () =>
    supabase
      .from('purchases')
...
export async function findExistingLegacyPayment(supabase, companyId, legacyPaymentId, cache = null) {
  ...
  const { data: byNotes } = await supabaseRead(`payments_dup_notes_${legacyPaymentId}`, () =>
    supabase
      .from('payments')
...
export async function findExistingLegacyExpense(supabase, companyId, legacyTransactionId, cache = null) {
  ...
  const { data: byDesc } = await supabaseRead(`expenses_dup_desc_${legacyTransactionId}`, () =>
    supabase
      .from('expenses')
