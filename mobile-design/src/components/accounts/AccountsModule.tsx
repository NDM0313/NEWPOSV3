...
export function AccountsModule({ onBack, user }: AccountsModuleProps) {
        ...
        <div className="p-4">
          <div className="grid grid-cols-2 gap-3">
            <ReportCard
              ...
              icon="📚"
              onClick={() => alert('Account Ledger - Coming Soon')}
            />
            <ReportCard
              title="Day Book"
            ...
            <ReportCard
              ...
              icon="💵"
              onClick={() => alert('Cash Summary - Coming Soon')}
            />
            <ReportCard
              title="Bank Summary"
              description="Bank accounts summary"
              icon="🏦"
              onClick={() => alert('Bank Summary - Coming Soon')}
            />
            <ReportCard
              title="Payables"
              description="Outstanding payables"
              icon="📤"
              onClick={() => alert('Payables Report - Coming Soon')}
            />
            <ReportCard
              title="Receivables"
              description="Outstanding receivables"
              icon="📥"
              onClick={() => alert('Receivables Report - Coming Soon')}
            />
          </div>
        </div>
      </div>
      ...
      <div className="min-h-screen pb-24 bg-[#111827]">
        <div className={`bg-gradient-to-br ${typeConfig.color} p-4 sticky top-0 z-10`}>
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1">
              <h1 className="font-semibold">{selectedEntry.entryNumber}</h1>
            ...
            <div className="flex items-center gap-3">
              <div className="w-12 h-12 bg-gradient-to-br from-[#8B5CF6] to-[#7C3AED] rounded-full flex items-center justify-center">
                <span className="text-white font-bold">
                  {selectedEntry.addedBy.split(' ').map(n => n[0]).join('')}
                </span>
              </div>
              <div className="flex-1">
...
function ReportCard({ title, description, icon, onClick }: ReportCardProps) {
  return (
    <button
      onClick={onClick}
      className="bg-[#1F2937] border border-[#374151] rounded-xl p-4 hover:border-[#6366F1] transition-all text-left"
    >
