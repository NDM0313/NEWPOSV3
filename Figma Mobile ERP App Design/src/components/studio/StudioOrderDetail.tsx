import { ArrowLeft, Plus, Edit2, CheckCircle, Clock, AlertCircle, Package } from 'lucide-react';
import { StudioOrder, StudioStage } from './StudioDashboard';
...
export function StudioOrderDetail({
  ...
  const getStageStatusConfig = (status: StudioStage['status']) => {
    switch (status) {
      case 'pending':
      ...
      <div className="bg-gradient-to-br from-[#8B5CF6] to-[#7C3AED] p-4 sticky top-0 z-10">
        ...
        <div className="grid grid-cols-3 gap-2">
          <div className="bg-white/10 rounded-lg p-2">
            ...
            <p className="text-sm font-semibold">{order.completedStages}/{order.totalStages}</p>
          </div>
          <div className="bg-white/10 rounded-lg p-2">
          ...
          <div className="bg-white/10 rounded-lg p-2">
            ...
            <p className="text-sm font-semibold capitalize">{order.status.replace('-', ' ')}</p>
          </div>
        </div>
      </div>
      ...
      <div className="p-4">
                  ...
                  >
                    ...
                    <div className="grid grid-cols-2 gap-3 mb-3">
                      <div>
                        ...
                        <p className="text-sm font-semibold text-[#EF4444]">Rs. {stage.internalCost.toLocaleString()}</p>
                      </div>
                      <div>
                        <p className="text-xs text-[#9CA3AF]">Customer Charge</p>
                        <p className="text-sm font-semibold text-[#10B981]">Rs. {stage.customerCharge.toLocaleString()}</p>
                      </div>
                      <div>
                    ...
                    {stage.status !== 'completed' && (
                      <button
                        ...
                        className="w-full py-2 bg-[#374151] hover:bg-[#8B5CF6] rounded-lg text-sm font-medium transition-colors"
                      >
        ...
        {canGenerateInvoice && (
          <button
            ...
            className="w-full mb-3 py-3 bg-gradient-to-r from-[#10B981] to-[#059669] rounded-xl font-semibold flex items-center justify-center gap-2 hover:opacity-90 transition-opacity"
          >
            <CheckCircle size={20} />
        ...
        {order.status === 'completed' && (
          <button
            onClick={onShipment}
            className="w-full py-3 bg-gradient-to-r from-[#8B5CF6] to-[#7C3AED] rounded-xl font-semibold flex items-center justify-center gap-2 hover:opacity-90 transition-opacity"
          >
            <Package size={20} />
            Process Shipment
          </button>
