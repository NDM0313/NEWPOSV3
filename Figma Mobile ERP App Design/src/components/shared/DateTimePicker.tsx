...
export function DateTimePicker({ value, onChange, onClose, showTime = false, label = 'DATE' }: DateTimePickerProps) {
  ...
  const [scrollMonth, setScrollMonth] = useState(selectedDate.getMonth());
  const [scrollDay, setScrollDay] = useState(selectedDate.getDate());
  const [scrollYear, setScrollYear] = useState(selectedDate.getFullYear());
  const [scrollHour, setScrollHour] = useState(selectedDate.getHours() % 12 || 12);
  const [scrollMinute, setScrollMinute] = useState(selectedDate.getMinutes());
  const [scrollPeriod, setScrollPeriod] = useState<'AM' | 'PM'>(selectedDate.getHours() >= 12 ? 'PM' : 'AM');
            ...
            {showTime && (
              <>
                ...
                <WheelPicker
                  ...
                  renderItem={(h) => h.toString().padStart(2, '0')}
                />
