...
interface AddContactFlowProps {
  onBack: () => void;
  onSubmit: (contact: Omit<Contact, 'id' | 'createdBy' | 'createdByRole' | 'createdAt'>) => void;
}

export function AddContactFlow({ onBack, onSubmit }: AddContactFlowProps) {
  const [formData, setFormData] = useState({
    ...
    balance: 0,
    creditLimit: 0,
    workerType: '' as 'dyer' | 'stitcher' | 'master' | 'handwork' | 'other' | '',
    workerRate: 0,
  ...
  const validate = () => {
    ...
    if (formData.roles.includes('worker') && !formData.workerType) {
  ...
  const handleSubmit = () => {
    ...
    const contact: Omit<Contact, 'id' | 'createdBy' | 'createdByRole' | 'createdAt'> = {
      name: formData.name,
  ...
  return (
    <div className="min-h-screen pb-40 bg-[#111827]">
      ...
      <div className="px-4 py-6 space-y-6">
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                className={`w-full h-12 bg-[#1F2937] border rounded-lg pl-10 pr-4 text-white placeholder:text-[#6B7280] focus:outline-none ${
                  errors.name
                    ? 'border-[#EF4444] focus:border-[#EF4444]'
                    : 'border-[#374151] focus:border-[#8B5CF6]'
                }`}
              />
            </div>
              ...
              <button
                ...
                className={`p-4 rounded-xl border-2 transition-all ${
                  formData.roles.includes(role)
                    ...
                    : 'bg-[#1F2937] border-[#374151] hover:border-[#8B5CF6]/50'
                }`}
              >
                ...
                <div className={`text-sm font-medium ${
                  formData.roles.includes(role) ? 'text-[#8B5CF6]' : 'text-[#9CA3AF]'
                }`}>
          ...
          <div className="space-y-4 bg-[#1F2937] border border-[#F59E0B]/30 rounded-xl p-4">
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => {
                  setFormData({ ...formData, workerType: e.target.value as any });
                  setErrors({ ...errors, workerType: '' });
                }}
                className={`w-full h-12 bg-[#111827] border rounded-lg px-4 text-white focus:outline-none ${
                  errors.workerType
                    ? 'border-[#EF4444] focus:border-[#EF4444]'
                    : 'border-[#374151] focus:border-[#F59E0B]'
                }`}
              >
        ...
        {/* Financial Information (Optional for Customer/Supplier) */}
