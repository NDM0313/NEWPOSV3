...
import { ArrowLeft, ShoppingCart, Calendar, Filter, ChevronRight, X, Search } from 'lucide-react';
...
type ReportType = 'daily' | 'customer' | 'product' | 'payment';
...
export function SalesReports({ onBack, user }: SalesReportsProps) {
  ...
  const calculateTotals = () => {
    ...
    const totalPaid = filteredSales.filter(s => s.paymentStatus === 'paid').reduce((sum, s) => sum + s.amount, 0);
    const totalPending = filteredSales.filter(s => s.paymentStatus === 'pending').reduce((sum, s) => sum + s.amount, 0);
  ...
  const handleExportPDF = async () => {
    try {
      ...
      const filename = `Sales_Report_${dateFrom}_to_${dateTo}.pdf`;
  ...
  const handleShare = async () => {
    try {
      ...
      const pdfBlob = await generateReportPDF(reportData);
      const filename = `Sales_Report_${dateFrom}_to_${dateTo}.pdf`;
      const shared = await sharePDF(pdfBlob, filename, 'Sales Report');
        ...
        <div className="p-4 space-y-4">
          ...
          <div className="bg-[#1F2937] border border-[#374151] rounded-xl p-4">
            <div className="flex items-center justify-between mb-4">
              ...
              <span className={`px-3 py-1 ${selectedRecord.paymentStatus === 'paid' ? 'bg-[#10B981]/10 text-[#10B981]' : selectedRecord.paymentStatus === 'partial' ? 'bg-[#F59E0B]/10 text-[#F59E0B]' : 'bg-[#EF4444]/10 text-[#EF4444]'} text-xs rounded-full font-medium`}>
            ...
            <div className="space-y-3">
              ...
              <div className="flex justify-between">
                ...
                <span className="text-sm text-white">{selectedRecord.quantity}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-sm text-[#9CA3AF]">Sale Type</span>
                <span className="text-sm text-white">{selectedRecord.saleType}</span>
              </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <button className="h-12 bg-[#3B82F6] hover:bg-[#2563EB] text-white rounded-lg font-medium transition-colors active:scale-95">
              Print Invoice
            </button>
            <button className="h-12 bg-[#1F2937] border border-[#374151] hover:border-[#3B82F6] text-white rounded-lg font-medium transition-colors active:scale-95">
              Share
            </button>
          </div>
        </div>
        ...
        {filteredSales.map((sale) => (
          ...
          >
            ...
            <div className="flex items-center gap-2">
              ...
              <span className={`px-2 py-1 ${sale.paymentStatus === 'paid' ? 'bg-[#10B981]/10 text-[#10B981]' : sale.paymentStatus === 'partial' ? 'bg-[#F59E0B]/10 text-[#F59E0B]' : 'bg-[#EF4444]/10 text-[#EF4444]'} text-xs rounded-md font-medium`}>
                {sale.paymentStatus}
              </span>
