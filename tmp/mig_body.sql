-- complete_bespoke_work_order: parent custom-order stock IN only for CUSTOM-* parents
-- or parents with fabric children / fabric_materials. Avoids inflating retail SKU stock
-- when a work order is attached to a simple (non-bespoke) sale line.
-- Production JE (Dr 5000 / Cr tailor AP) is unchanged.
...
DECLARE
  ...
  v_has_fabric_materials boolean := false;
  v_pid uuid;
  v_vid uuid;
  v_qty numeric;
  v_name text;
...
BEGIN
  ...
  IF v_wo.journal_entry_id IS NULL AND v_wo.status <> 'completed'::public.bespoke_work_order_status THEN
    ...
    v_desc := 'Bespoke WO ' || COALESCE(v_wo.work_order_no, p_work_order_id::text)
           || COALESCE(' â€” Sale ' || v_wo.sale_invoice_no, '');
    ...
    VALUES
      (v_journal_id, v_cost_account_id, v_wo.production_cost, 0, 'Bespoke production cost â€” ' || v_wo.work_order_no),
      (v_journal_id, v_ap_account_id, 0, v_wo.production_cost, 'Tailor payable â€” ' || v_wo.work_order_no);
  ...
  LOOP
    ...
    IF NOT EXISTS (
      ...
      INSERT INTO public.stock_movements (
        ...
        'Bespoke fabric OUT â€” ' || COALESCE(v_wo.work_order_no, p_work_order_id::text) || ' â€” ' || v_name,
        p_user_id, now()
      );
  ...
  -- Parent custom-order line IN â€” only CUSTOM-* or fabric-linked parents (not plain retail SKUs)
  SELECT
    si.product_id,
    si.variation_id,
    si.quantity,
  ...
  IF v_pid IS NOT NULL
     AND COALESCE(v_qty, 0) > 0
     AND (
       COALESCE(v_sku, '') ILIKE 'CUSTOM-%'
       OR v_has_child_fabric
       OR v_has_fabric_materials
     )
  THEN
    ...
    IF NOT EXISTS (
      ...
      INSERT INTO public.stock_movements (
        ...
        v_qty, v_unit_cost, v_qty * v_unit_cost,
        'sale', 'bespoke_work_order', p_work_order_id,
        'Bespoke custom order IN â€” ' || COALESCE(v_wo.work_order_no, p_work_order_id::text) || ' â€” ' || v_name,
        p_user_id, now()
      );
      v_stock_posted := v_stock_posted + 1;
    END IF;
...
COMMENT ON FUNCTION public.complete_bespoke_work_order(uuid, uuid) IS
  'Complete bespoke WO: Dr 5000 / Cr tailor AP + fabric OUT; parent stock IN only for CUSTOM-* or fabric-linked parents.';
