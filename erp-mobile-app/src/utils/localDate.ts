...
/** Whether a date input value includes an explicit time component. */
export function inputHasTime(value: string | null | undefined): boolean {
  const s = String(value ?? '').trim();
  return s.includes('T') && /T\d{1,2}:\d{2}/.test(s);
}

/** Split YYYY-MM-DD or YYYY-MM-DDTHH:mm into parts. */
export function splitDateTimeInput(value: string | null | undefined): {
  date: string;
  time: string | null;
  hasTime: boolean;
} {
  const s = String(value ?? '').trim();
  if (!s) {
    const now = localNowDateString();
    return { date: now, time: null, hasTime: false };
  }
  const [datePart, timePart] = s.split('T');
  const date = /^\d{4}-\d{2}-\d{2}$/.test(datePart) ? datePart : toLocalDateString(s);
  const hasTime = Boolean(timePart && /^\d{1,2}:\d{2}/.test(timePart));
  return { date, time: hasTime ? timePart.slice(0, 5) : null, hasTime };
}

/** Date-only value for `<input type="date">`. */
export function toDateInputValue(value: string | null | undefined): string {
  return splitDateTimeInput(value).date;
}

/** Value for `<input type="datetime-local">` (defaults time to now when missing). */
export function toDateTimeInputValue(value: string | null | undefined): string {
  const { date, time, hasTime } = splitDateTimeInput(value);
  if (hasTime && time) return `${date}T${time}`;
  const now = new Date();
  return `${date}T${pad2(now.getHours())}:${pad2(now.getMinutes())}`;
}

/** Document/payment write: date column gets YYYY-MM-DD; optional full timestamp when time set. */
export function documentDateFromInput(value: string | null | undefined): {
  dateOnly: string;
  timestamp: string | null;
} {
  const { date, hasTime } = splitDateTimeInput(value);
  if (!hasTime) return { dateOnly: date, timestamp: null };
  const d = parseLocalDateInput(value);
  return {
    dateOnly: date,
    timestamp: Number.isNaN(d.getTime()) ? null : toLocalISOString(d),
  };
}

/** Inclusive query bounds for date-range filters (date columns + optional timestamp narrow). */
export function normalizeDateRangeInput(
  ...
  const dateFrom = fromTrim ? toLocalDateString(fromTrim) : undefined;
  const dateTo = toTrim ? toLocalDateString(toTrim) : undefined;
  const fromHasTime = fromTrim ? inputHasTime(fromTrim) : false;
  const toHasTime = toTrim ? inputHasTime(toTrim) : false;

  let timestampFrom: string | undefined;
  let timestampTo: string | undefined;

  if (fromTrim && fromHasTime) {
    const d = parseLocalDateInput(fromTrim);
    if (!Number.isNaN(d.getTime())) timestampFrom = toLocalISOString(d);
  } else if (dateFrom) {
    timestampFrom = toLocalISOString(parseLocalDateInput(`${dateFrom}T00:00:00`));
  }

  if (toTrim && toHasTime) {
    const d = parseLocalDateInput(toTrim);
    if (!Number.isNaN(d.getTime())) timestampTo = toLocalISOString(d);
  } else if (dateTo) {
    const end = parseLocalDateInput(`${dateTo}T23:59:59`);
    timestampTo = toLocalISOString(end);
  }

  return { dateFrom, dateTo, timestampFrom, timestampTo };
}

/** @deprecated alias */
export function rangeBoundsForQuery(from?: string | null, to?: string | null) {
  return normalizeDateRangeInput(from, to);
}
