...
export function isLiquidityPaymentAccount(acc: PaymentAccountRef | null | undefined): boolean {
  if (!acc) return false;
...
export async function enrichExpenseRowsWithPostedPaymentAccount(
  ...
  rows.forEach((row) => {
    ...
    if (resolved?.accountId && resolved.account) {
      row.posted_payment_account_id = resolved.accountId;
      row.posted_payment_account = resolved.account;
      row.posted_payment_display = resolved.display;
    } else if (resolved) {
      row.posted_payment_display = resolved.display;
    }
  });
...
export function postedPaymentDisplayFromRow(row: ExpenseRowLike): string {
...
export function postedPaymentAccountIdFromRow(row: ExpenseRowLike): string | null {
