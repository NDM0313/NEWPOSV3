...
/** Prefer exact SKU hits; when any rank-0 exists, keep ranks 0–2 only. */
...
/** Default name/SKU/barcode match for pickers. */
export function productMatchesSearch(
  product: { name?: string | null; sku?: string | null; barcode?: string | null; description?: string | null },
  searchTerm: string,
): boolean {
  const term = String(searchTerm || '').trim();
  if (!term) return true;
  if (product.barcode && product.barcode.toLowerCase().includes(term.toLowerCase())) return true;
  return rankProductSearchHit(product, term) < 99;
}

export function filterAndRankProducts<T extends { name?: string | null; sku?: string | null; description?: string | null }>(
  items: T[],
  searchTerm: string,
  matches: (item: T, term: string) => boolean = productMatchesSearch as (item: T, term: string) => boolean,
  options?: { cap?: number; preferExact?: boolean },
): T[] {
  const term = String(searchTerm || '').trim();
  if (!term) return items;
  let results = items.filter((item) => matches(item, term));
  results.sort((a, b) => {
  ...
  if (options?.preferExact !== false) {
    results = preferExactSkuHits(results, term);
  }
  const cap = options?.cap ?? PRODUCT_SEARCH_RESULT_CAP;
  if (cap > 0 && results.length > cap) {
