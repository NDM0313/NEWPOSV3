        ...
        {TABS.map((tab) => {
          ...
          if (tab.isCenter) {
            return (
              <button
                ...
                className="relative -mt-6 flex flex-col items-center justify-center w-16 h-16 rounded-full shadow-lg bg-[#3B82F6] text-white hover:bg-[#2563EB] active:scale-95 transition-all touch-manipulation"
          ...
          return (
            <button
              ...
              type="button"
              onClick={() => onTabChange(tab.id)}
              className="flex flex-col items-center justify-center gap-1.5 py-4 px-3 min-w-[72px] min-h-[56px] transition-colors touch-manipulation rounded-lg active:bg-[#374151]/50"
              aria-label={tab.label}
            >
              <Icon
                className={`w-7 h-7 ${isActive ? 'text-[#3B82F6]' : 'text-[#9CA3AF]'}`}
