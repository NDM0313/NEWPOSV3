...
export interface RentalAddAttachmentsSheetProps {
  open: boolean;
  companyId: string;
  rentalId: string;
  existingRaw: unknown;
...
export function RentalAddAttachmentsSheet({
  open,
  companyId,
  rentalId,
  existingRaw,
  ...
  onClose,
  onSaved,
}: RentalAddAttachmentsSheetProps) {
  ...
  const handleSave = async () => {
    ...
    setError(null);
    try {
      const res = await rentalsApi.appendRentalAttachments(companyId, rentalId, pendingFiles, existing);
      if (res.error) {
        setError(res.error);
        return;
      }
  ...
  const title = existing.length > 0 ? 'Update attachments' : 'Add attachments';
      ...
      >
        <div className="flex items-center justify-between px-4 py-3 border-b border-[#374151]">
          <div>
            ...
            {bookingLabel ? (
              <p className="text-xs text-[#9CA3AF] truncate max-w-[240px]">{bookingLabel}</p>
            ) : null}
          </div>
          <button
            type="button"
        ...
        <div className="p-4 border-t border-[#374151]">
          <button
            ...
            className="w-full h-11 rounded-lg bg-[#8B5CF6] hover:bg-[#7C3AED] disabled:opacity-50 text-white font-medium flex items-center justify-center gap-2"
          >
