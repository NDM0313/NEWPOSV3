...
import { ArrowLeft, Loader2, Building2, Share2, FileDown, RefreshCw } from 'lucide-react';
...
export function BankSummaryReport({ onBack, companyId }: BankSummaryReportProps) {
  ...
  const fetchAccounts = () => {
    ...
    accountsApi.getAccounts(companyId).then(({ data, error }) => {
      ...
      setAccounts(all.filter((a) => (a.type || '').toLowerCase() === 'bank'));
    });
  };
  ...
  const handleShare = () => {
    const lines = [`Bank Summary: Total Rs. ${total.toLocaleString()}`, `${accounts.length} account(s)`];
    accounts.forEach((a) => lines.push(`${a.name} (${a.code}): Rs. ${a.balance.toLocaleString()}`));
    if (navigator.share) {
      navigator.share({ title: 'Bank Summary', text: lines.join('\n') }).catch(() => {});
    }
  };
    ...
    <div className="min-h-screen bg-[#111827] pb-24">
      <div className="bg-gradient-to-br from-[#3B82F6] to-[#2563EB] p-4 sticky top-0 z-10">
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            <h1 className="font-semibold text-white">Bank Summary</h1>
            <p className="text-xs text-white/80">Bank accounts balance</p>
          </div>
      ...
      <div className="p-4 space-y-4">
        <div className="bg-[#1F2937] border border-[#374151] rounded-xl p-5 flex items-center gap-4">
          ...
          <div>
            <p className="text-sm text-[#9CA3AF]">Total Bank</p>
        ...
        {loading ? (
          ...
          <div className="space-y-2">
            <h2 className="text-sm font-medium text-[#9CA3AF]">Bank accounts</h2>
            {accounts.map((a) => (
              <div
                key={a.id}
        ...
        {!loading && accounts.length === 0 && (
          <div className="text-center py-12 text-[#9CA3AF]">
            <Building2 className="w-16 h-16 mx-auto mb-4 text-[#374151]" />
