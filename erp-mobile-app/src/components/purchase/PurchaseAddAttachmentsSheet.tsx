...
export interface PurchaseAddAttachmentsSheetProps {
  open: boolean;
  companyId: string;
  purchaseId: string;
  existingRaw: unknown;
...
export function PurchaseAddAttachmentsSheet({
  open,
  companyId,
  purchaseId,
  existingRaw,
  ...
  onClose,
  onSaved,
}: PurchaseAddAttachmentsSheetProps) {
  ...
  const handleSave = async () => {
    ...
    setError(null);
    try {
      const res = await purchasesApi.appendPurchaseAttachments(
        companyId,
        purchaseId,
        pendingFiles,
        existing,
      ...
      >
        <div className="flex items-center justify-between px-4 py-3 border-b border-[#374151]">
          <div>
            ...
            {documentLabel ? (
              <p className="text-xs text-[#9CA3AF] truncate max-w-[240px]">{documentLabel}</p>
            ) : null}
          </div>
          <button
            type="button"
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3">
          ...
          <SaleAttachmentEditor
            ...
            disabled={saving}
            maxTotal={purchasesApi.MAX_PURCHASE_ATTACHMENTS_COUNT}
            compact
          />
        </div>
