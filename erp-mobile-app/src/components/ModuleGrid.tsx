...
export function ModuleGrid({ onClose, onModuleSelect, userRole }: ModuleGridProps) {
  ...
  return (
    <>
      <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-40" onClick={onClose} aria-hidden />
      <div className="fixed inset-x-0 bottom-0 bg-[#1F2937] rounded-t-3xl z-50 animate-slide-up max-h-[80vh] overflow-hidden">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#374151]">
          <div>
            ...
            <p className="text-sm text-[#9CA3AF]">{enabled.length} available</p>
          </div>
          <button onClick={onClose} className="p-2 hover:bg-[#374151] rounded-lg transition-colors text-white">
            <X className="w-5 h-5" />
          </button>
        </div>
        <div className="p-6 overflow-y-auto max-h-[calc(80vh-120px)]">
          <div className="grid grid-cols-3 gap-4">
            {enabled.map((module) => (
              <button
                ...
                className="flex flex-col items-center gap-3 p-4 bg-[#111827] border border-[#374151] rounded-2xl hover:border-[#3B82F6] active:scale-95 transition-all text-white"
              >
                <div className="w-12 h-12 rounded-xl flex items-center justify-center" style={{ backgroundColor: `${module.color}20`, color: module.color }}>
                  {module.icon}
                </div>
          ...
          <div className="mt-6 p-4 bg-[#111827] border border-[#374151] rounded-xl">
            <p className="text-xs text-[#9CA3AF] text-center">Modules shown based on your role permissions</p>
          </div>
        </div>
      </div>
    </>
