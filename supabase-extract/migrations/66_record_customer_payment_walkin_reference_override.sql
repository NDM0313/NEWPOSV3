-- record_customer_payment: walk-in contact when sale.customer_id is null; optional reference override (mobile)
-- Replace 9-arg overload so PostgREST resolves a single function.

DROP FUNCTION IF EXISTS public.record_customer_payment(UUID, UUID, UUID, NUMERIC, UUID, TEXT, DATE, TEXT, UUID);
...
CREATE OR REPLACE FUNCTION public.record_customer_payment(
  ...
  p_reference_number_override TEXT DEFAULT NULL
)
RETURNS JSON
LANGUAGE plpgsql
SECURITY DEFINER
...
DECLARE
  ...
  v_override_nonempty BOOLEAN := false;
BEGIN
  ...
  IF v_customer_id IS NULL THEN
    ...
    WHERE c.company_id = p_company_id
      ...
      AND (
        c.system_type = 'walking_customer'
        OR (c.is_default = true AND COALESCE(c.is_system_generated, false) = true)
      )
    ORDER BY CASE WHEN c.system_type = 'walking_customer' THEN 0 ELSE 1 END
    LIMIT 1;
  END IF;
  ...
  IF v_customer_id IS NULL THEN
    RETURN json_build_object(
      'success', false,
      'error', 'Sale has no customer and no walk-in contact for this company. Add a customer or ensure walk-in exists.'
    );
  END IF;
  ...
  v_override_nonempty := COALESCE(TRIM(p_reference_number_override), '') <> '';

  LOOP
    v_try := v_try + 1;

    BEGIN
      IF v_try = 1 AND v_override_nonempty AND NOT v_override_used THEN
        v_ref_no := LEFT(TRIM(p_reference_number_override), 100);
...
COMMENT ON FUNCTION public.record_customer_payment(UUID, UUID, UUID, NUMERIC, UUID, TEXT, DATE, TEXT, UUID, TEXT) IS
  'Customer receipt: walk-in contact when sale has no customer_id; optional p_reference_number_override on first attempt.';
