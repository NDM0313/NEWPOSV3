-- ============================================================================
-- Migration 59: Business type + modules in Create Business Wizard
-- - companies.business_type (retail, wholesale, manufacturing, rental, mixed)
-- - create_business_transaction accepts p_business_type and p_modules (JSONB)
-- - Inserts enabled modules into modules_config
-- ============================================================================

-- Add business_type to companies (nullable for existing rows)
ALTER TABLE companies ADD COLUMN IF NOT EXISTS business_type VARCHAR(50);

-- Replace function only if current user owns it (otherwise column is enough; run as postgres to update RPC)
DO $$
BEGIN
  DROP FUNCTION IF EXISTS create_business_transaction(TEXT, TEXT, TEXT, TEXT, UUID, TEXT, DATE, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT);
EXCEPTION WHEN OTHERS THEN
  RAISE NOTICE '59: Skip drop create_business_transaction (%). Run as postgres to update.', SQLERRM;
END $$;
...
BEGIN
  ...
  INSERT INTO users (id, company_id, email, full_name, role, is_active, created_at, updated_at)
  VALUES (v_user_id, v_company_id, p_email, p_owner_name, 'admin', true, NOW(), NOW());
...
$body$;
$m59$;
  RAISE NOTICE '59: create_business_transaction replaced (14-param with business_type and modules).';
EXCEPTION WHEN OTHERS THEN
  RAISE NOTICE '59: Could not replace create_business_transaction. Column business_type is applied. Run this migration as postgres to update RPC: %', SQLERRM;
END $$;
...
BEGIN
  ...
  COMMENT ON FUNCTION create_business_transaction(TEXT, TEXT, TEXT, TEXT, UUID, TEXT, DATE, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, JSONB) IS 'Creates business (company, branch, user) with address, phone, country, timezone, business_type. Writes enabled modules to modules_config.';
EXCEPTION WHEN OTHERS THEN
  RAISE NOTICE '59: Grants/comment skipped: %', SQLERRM;
END $$;