-- Issue 02: Customer ledger must reflect shipping. Return shipment_charges so app can show sale debit = total + shipment_charges.
...
STABLE
AS $$
  SELECT s.id, s.invoice_no, s.invoice_date, s.total,
         COALESCE(s.shipment_charges, 0),
...
COMMENT ON FUNCTION get_customer_ledger_sales(UUID,UUID,DATE,DATE) IS 'Ledger: sales by company_id + customer_id; includes shipment_charges for correct receivable (total + shipment).';
