-- ============================================================================
-- Company transaction reset: hard-reset document counters after transactional wipe
-- ----------------------------------------------------------------------------
-- Ensures next PUR-/SL-/JE-/etc. is truly #1 by:
--   • DELETE + re-seed document_sequences_global with current_number = 0
--     (first get_next_document_number_global increments → 1).
--   • DELETE + re-seed erp_document_sequences (sentinel branch, current year)
--     with last_number = 0 (first generate_document_number increments → 1).
--   • Temporarily SET row_security = off so RLS cannot block maintenance.
--
-- Limitation: legacy rentals.rental_no via trigger set_rental_no() uses the
-- GLOBAL sequence rental_no_seq; that is NOT per-company and is NOT reset here.
-- Use booking_no + RNT / ERP RENTAL numbering for per-company reset semantics.
-- ============================================================================
...
BEGIN
  ...
  -- Numbering counters: delete, then re-seed at 0 so next RPC call yields *0001.
