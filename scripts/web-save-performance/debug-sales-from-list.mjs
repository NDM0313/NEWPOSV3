...
const count = await page.getByRole('button', { name: /Search products by name, SKU/i }).count();
const visible = await page.getByRole('button', { name: /Search products by name, SKU/i }).first().isVisible().catch(() => false);
console.log(JSON.stringify({ searchButtonCount: count, visible }, null, 2));
await browser.close();
