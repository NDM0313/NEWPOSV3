#!/bin/bash
# Phase 2.13 — production frontend deploy ONLY (Party Ledger loader OFF).
set -euo pipefail

DEPLOY_DIR="${DEPLOY_DIR:-/root/NEWPOSV3}"
BUILD_LABEL="${BUILD_LABEL:-phase-213-prod}"
ROLLBACK_TAG="erp-frontend:rollback-before-213-$(date +%Y%m%d%H%M%S)"

cd "$DEPLOY_DIR"

docker tag erp-frontend:latest "$ROLLBACK_TAG" 2>/dev/null || true
echo "[phase-213] rollback tag: $ROLLBACK_TAG"
...
echo "[phase-213] Confirm Party Ledger loader OFF..."
docker exec -i supabase-db psql -U postgres -d postgres -t -A -c \
  "SELECT feature_key, enabled FROM feature_flags WHERE company_id = '30bd8592-3384-4f34-899a-f3907e336485' AND feature_key IN ('unified_ledger_loader_party_ledger','unified_ledger_screen_party_ledger');"
...
echo "[phase-213] Bundle check:"
docker exec erp-frontend sh -c '
  cd /usr/share/nginx/html/assets &&
  for s in data-party-ledger-main-loader resolvePartyLedgerMainLoaderSource unified_ledger_loader_party_ledger; do
    if grep -rl "$s" . >/dev/null 2>&1; then echo "FOUND:$s"; else echo "MISSING:$s"; fi
  done
' || true

echo "[phase-213] DONE production frontend. Party Ledger loader NOT enabled."
