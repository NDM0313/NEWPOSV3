#!/bin/bash
# Phase 2.14 — production frontend deploy ONLY (Roznamcha loader OFF).
set -euo pipefail

DEPLOY_DIR="${DEPLOY_DIR:-/root/NEWPOSV3}"
BUILD_LABEL="${BUILD_LABEL:-phase-214-prod}"
ROLLBACK_TAG="erp-frontend:rollback-before-214-$(date +%Y%m%d%H%M%S)"

cd "$DEPLOY_DIR"

docker tag erp-frontend:latest "$ROLLBACK_TAG" 2>/dev/null || true
echo "[phase-214] rollback tag: $ROLLBACK_TAG"
...
echo "[phase-214] Confirm Roznamcha loader OFF..."
...
echo "[phase-214] Bundle check:"
docker exec erp-frontend sh -c "cd /usr/share/nginx/html/assets && for s in data-roznamcha-main-loader resolveRoznamchaMainLoaderSource unified_ledger_loader_roznamcha; do if grep -rl \"\$s\" . >/dev/null 2>&1; then echo FOUND:\$s; else echo MISSING:\$s; fi; done" || true

echo "[phase-214] DONE production frontend. Roznamcha loader NOT enabled."
