...
test('loadMonitoringProfile din-china defaults', () => {
  ...
  assert.equal(p.golden.trialBalanceTotal, 333_268_801.7);
  assert.equal(p.golden.roznamcha.cashIn, 59_762_230);
  assert.equal(p.golden.roznamcha.cashOut, 35_651_877);
  assert.equal(p.golden.roznamcha.closing, 24_110_353);
